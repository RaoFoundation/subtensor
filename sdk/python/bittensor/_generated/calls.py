...
class Scheduler:
    ...
    def cancel(when: 'u32', index: 'u32') -> Call:
        "Cancel a scheduled task (named or anonymous), by providing the block it is scheduled for execution in, as well as the index of the task in that block's agenda.  In the case of a named task, it will remove it from the lookup table as well."
    ...
    def set_retry(task: 'Any', retries: 'u8', period: 'u32') -> Call:
        "Set a retry configuration for a task so that, in case its scheduled run fails, it will be retried after `period` blocks, for a total amount of `retries` retries or until it succeeds.  Tasks which need to be scheduled for a retry are still subject to weight metering and agenda space, same as a regular task. If a periodic task fails, it will be scheduled normally while the task is retrying.  Tasks scheduled as a result of a retry for a periodic task are unnamed, non-periodic clones of the original task. Their retry configuration will be derived from the original task's configuration, but will have a lower value for `remaining` than the original `total_retries`.  This call **cannot** be used to set a retry configuration for a named task."
    ...
    def set_retry_named(id: 'Any', retries: 'u8', period: 'u32') -> Call:
        "Set a retry configuration for a named task so that, in case its scheduled run fails, it will be retried after `period` blocks, for a total amount of `retries` retries or until it succeeds.  Tasks which need to be scheduled for a retry are still subject to weight metering and agenda space, same as a regular task. If a periodic task fails, it will be scheduled normally while the task is retrying.  Tasks scheduled as a result of a retry for a periodic task are unnamed, non-periodic clones of the original task. Their retry configuration will be derived from the original task's configuration, but will have a lower value for `remaining` than the original `total_retries`.  This is the only way to set a retry configuration for a named task."
