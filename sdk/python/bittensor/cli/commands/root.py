...
network, and ``swap`` rebalances a validator's basket between subnets —
the only way a basket's composition changes; dividends accumulate in place.
"""
...
from . import root_move, root_swap

app = typer.Typer(
    no_args_is_help=True,
    help="Root network: validator baskets, basket swaps, and your TAO positions."
...
app.command("swap")(root_swap.root_swap)
...
def root_register(
    ...
    is what lets the hotkey receive root stake; its dividends accumulate as
    alpha on the subnets they are earned on, and it reshapes that basket
    with `btcli root swap`.
    """
    app_ctx: AppContext = ctx_of(ctx)
    hotkey = app_ctx.resolve_address("hotkey_ss58", hotkey_ss58)
