...
Grouped by the pallet that (first) declares the error, mirroring the layout of
``error_map.py``. Names shared across pallets carry one description covering
all meanings.
"""
...
DESCRIPTIONS: dict[str, str] = {
    ...
    "CannotAffordLockCost": (
        ...
        "Compare `btcli subnets lock-cost` (or `get_network_lock_cost`) against the coldkey "
        "balance from `btcli wallet balance` before registering a subnet."
    ),
    ...
    "CommitRevealDisabled": (
        ...
        "Check the `commit_reveal_weights_enabled` hyperparameter for the netuid (`btcli sudo "
        "get`); use plain `set_weights` instead when it is disabled."
    ),
    "CommitRevealEnabled": (
    ...
    "HotKeyNotRegisteredInNetwork": (
        ...
        "registration on any subnet. Verify registration with `btcli subnets show --netuid <n>` "
        "and register via `btcli subnets register` first."
    ),
    "HotKeyNotRegisteredInSubNet": (
        "The hotkey holds no UID on the given netuid, so weight setting, commits, or UID "
        "lookups fail there. Check the `Uids` storage for the (netuid, hotkey) pair or `btcli "
        "subnets show --netuid <n>`; confirm the netuid argument and register the hotkey if "
        "needed."
    ),
    ...
    "IncorrectWeightVersionKey": (
        ...
        "weights version. Compare it against the `WeightsVersionKey` hyperparameter (`btcli "
        "subnets hyperparameters`) and update the validator software or the key."
    ),
    "InputLengthsUnequal": (
    ...
    "LiquidAlphaDisabled": (
        ...
        "subnet. Check the `LiquidAlphaOn` hyperparameter (`btcli subnets hyperparameters`) and "
    ...
    "MaxWeightExceeded": (
        ...
        "limit (self-weight is exempt). Check the `MaxWeightsLimit` hyperparameter (`btcli "
        "subnets hyperparameters`) and flatten the weight vector before setting."
    ),
    ...
    "NotSubnetOwner": (
        ...
        "against `SubnetOwner` for that netuid (`btcli subnets show`)."
    ),
    "Overflow": (
    ...
    "StakeTooLowForRoot": (
        ...
        "against existing root validators (`btcli root list`)."
    ),
    "StakeUnavailable": (
