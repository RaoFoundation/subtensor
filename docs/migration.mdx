...
3. Watch the semantic traps in [Money](#money-and-balance),
   [Weights and take](#weights-children-and-take), and
   [Results and errors](#results-and-errors) — these change behavior, not
   just names.
...
1. **One client, no method zoo.** `Subtensor` / `AsyncSubtensor` become
   `sub.Client` (async) or `sub.SyncClient` (blocking, same surface). Reads
   live on typed namespaces (`client.balances`, `client.subnets`,
   `client.neurons`, `client.staking`), on the named-read catalog
   (`client.read("uid", hotkey_ss58=..., netuid=...)`), or on generic
   accessors (`client.query(sub.storage...)`). See
...
General rules: `client.read(name, ...)` takes the same `..._ss58` parameter
names the old methods used; results that used to be raw u16/u64 integers come
back normalized (weights and bonds as 0..1 floats, balances as `Balance`).
...
| `get_stake_info_for_coldkey(coldkey_ss58)` | `await client.read("stake_for_coldkey", coldkey_ss58=...)` → `list[StakePosition]` |
| `get_stake_info_for_coldkeys([...])` | `client.read("stake_for_coldkeys", coldkey_ss58s=[...])` |
| `get_stake_for_coldkey_and_hotkey(...)` | `client.read("stake", ...)` per netuid, or filter `stake_for_coldkey` |
| `get_staking_hotkeys(coldkey_ss58)` | `client.read("staking_hotkeys", coldkey_ss58=...)` |
| `get_auto_stakes(coldkey_ss58)` | `client.read("auto_stake_all", coldkey_ss58=...)` |
| valuing stake in TAO (manual sum) | `client.read("stake_value_for_coldkey", coldkey_ss58=...)` — never sum alpha yourself ([Money](/docs/concepts/money)) |
| `sim_swap(...)`, `get_stake_add_fee(...)` | `client.read("quote_stake", netuid=..., amount_tao=...)` |
| `get_unstake_fee(...)`, `get_stake_movement_fee(...)` | `client.read("quote_unstake", netuid=..., amount_alpha=...)` |
| `get_transfer_fee(...)`, `get_extrinsic_fee(...)` | `(await client.plan(intent, wallet)).fee` |
| `get_stake_lock` / `get_coldkey_lock` | `client.read("coldkey_lock", coldkey_ss58=..., netuid=...)` |
| `get_stake_locks(...)` | `client.read("locks_for_coldkey", coldkey_ss58=...)` |
| `get_hotkey_conviction(hotkey_ss58, netuid)` | `client.read("hotkey_conviction", ...)` |
| `get_most_convicted_hotkey_on_subnet(netuid)` | `client.read("most_convicted_hotkey", netuid=...)` |
...
| `subnet_exists(netuid)` | `client.read("subnet", netuid=...)` and check |
| `get_subnet_burn_cost()` | `client.read("subnet_registration_cost")` |
| `recycle(netuid)` (registration price) | `await client.subnets.burn(netuid)` / `client.read("burn", ...)` |
| `get_subnet_price(netuid)` / `get_subnet_prices()` | `client.read("alpha_price", netuid=...)` / `client.read("alpha_prices")` |
| `get_subnet_hyperparameters(netuid)` | `client.read("subnet_hyperparameters", netuid=...)` |
| `difficulty` / `immunity_period` / `min_allowed_weights` / `max_weight_limit` / `weights_rate_limit` | same names as reads: `client.read("immunity_period", netuid=...)` etc. |
| `commit_reveal_enabled(netuid)` | `client.subnets.commit_reveal_enabled(netuid)` |
| `get_subnet_reveal_period_epochs(netuid)` | `client.read("reveal_period", netuid=...)` |
| `tempo(netuid)` / `subnetwork_n(netuid)` | fields on `client.subnets.info(netuid)` / the metagraph |
| `blocks_since_last_step` / `blocks_until_next_epoch` / `get_next_epoch_start_block` | same names as reads (`client.read("blocks_until_next_epoch", netuid=...)`) |
| `blocks_since_last_update(netuid, uid)` | `client.read("blocks_since_last_update", netuid=..., uid=...)` |
| `is_subnet_active(netuid)` | `client.read("epoch_status", netuid=...)` / `client.read("subnet_start_schedule", netuid=...)` |
| `get_mechanism_count` / `get_mechanism_emission_split` | `client.read("mechanism_count", ...)` / `client.read("mechanism_emission_split", ...)` |
| `weights(netuid, mechid)` | `client.read("weights", netuid=..., mechid=0)` — now `dict[uid, dict[uid, float]]` normalized 0..1, **not** u16 tuples |
| `bonds(netuid, mechid)` | `client.read("bonds", netuid=..., mechid=0)` — same shape change |
| `get_timelocked_weight_commits(...)` | `client.read("timelocked_weight_commits", netuid=..., mechid=0)` |
...
| `get_uid_for_hotkey_on_subnet(hotkey_ss58, netuid)` | `client.read("uid", hotkey_ss58=..., netuid=...)` |
| `is_hotkey_registered` / `..._on_subnet` / `..._any` | `client.read("uid", ...)` / `client.read("netuids_for_hotkey", ...)` |
| `get_netuids_for_hotkey(hotkey_ss58)` | `client.read("netuids_for_hotkey", hotkey_ss58=...)` |
| `get_hotkey_owner(hotkey_ss58)` / `does_hotkey_exist` | `client.read("hotkey_owner", hotkey_ss58=...)` |
| `get_owned_hotkeys(coldkey_ss58)` | `client.read("owned_hotkeys", coldkey_ss58=...)` |
| `get_delegates()` / `get_delegate_by_hotkey` / `is_hotkey_delegate` / `get_delegate_take` / `get_delegated` | reads `delegates`, `delegate`, `is_delegate`, `delegate_take`, `delegated` |
| `get_delegate_identities()` | `client.read("hotkey_identities", hotkey_ss58s=[...])` |
| `get_children` / `get_children_pending` / `get_parents` | reads `children`, `pending_children`, `parents` |
| `query_identity(coldkey_ss58)` | `client.read("identity", coldkey_ss58=...)` |
| `get_commitment` / `get_commitment_metadata` | `client.read("commitment", netuid=..., hotkey_ss58=...)` |
| `get_all_commitments(netuid)` | `await client.subnets.commitments(netuid)` |
| `get_revealed_commitment*` | `client.read("revealed_commitment", netuid=..., hotkey_ss58=...)` |
...
| `tx_rate_limit()` / `is_fast_blocks()` | reads `tx_rate_limit`, `is_fast_blocks` |
| `last_drand_round()` | `sub.timelock.current_round()` |
| `get_proxies_for_real_account(ss58)` | `client.read("proxies", coldkey_ss58=...)` |
| `get_coldkey_swap_announcement(ss58)` | `client.read("coldkey_swap_announcement", coldkey_ss58=...)` |
| `get_crowdloans()` / `get_crowdloan_by_id` / `get_crowdloan_contributions` | reads `crowdloans`, `crowdloan`, `crowdloan_contributors` |
| `get_mev_shield_next_key()` | `client.read("mev_shield_next_key")` |
| `get_root_claim_type(ss58)` | `client.read("root_claim_type", coldkey_ss58=...)` |
...
| `mg.W` (weight matrix) | `await client.read("weights", netuid=...)` |
| `mg.B` (bond matrix) | `await client.read("bonds", netuid=...)` |
...
4. Rewrite each transaction as an intent + `client.execute`; take values and
   ...
   u16/u64 wire values); replace safe-staking flags with `*Limit` intents.
5. Update result handling: `.success` still works, but branch on
   `result.error.code` instead of exception types or message strings.
...
10. Dry-run everything: `client.plan(...)` in Python, `--dry-run` on the
