...
| `get_root_claim_type(ss58)` | removed (retired on chain in v438; claims are always TAO to root stake) |
| other `get_root_claim*` reads | `client.staking.root_basket_owed(coldkey_ss58=...)` and the other [beta basket reads](/docs/guides/root-dividends) |
...
| `claim_root(wallet, netuids)` | `bt.ClaimRoot()` — parameterless since v438 (fund-level, [beta baskets](/docs/guides/root-dividends)) |
| `set_root_claim_type(wallet, type)` | removed (retired on chain in v438; claims are always TAO to root stake) |
...
| `stake set-claim` / `process-claim` | `stake claim` (parameterless since v438; `set-claim` removed — see [root dividends](/docs/guides/root-dividends)) |
