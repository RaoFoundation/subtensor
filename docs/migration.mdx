...
- **Weights** are easier: pass floats (any scale — only proportions matter)
  ...
  one.
- **Take** was a float 0–1 (`take=0.18`); it is now a u16 proportion:
  `take=int(0.18 * 65535)`. `set_delegate_take`'s auto-direction behavior is
  preserved by `sub.SetTake`; `sub.IncreaseTake` / `sub.DecreaseTake` pin the
  direction.
- **Children proportions** were floats 0–1; they are now u64 shares:
  `children=[[int(0.5 * (2**64 - 1)), "5F...child"]]`.
...
the same machinery. Full details: [The CLI](/docs/cli).
...
4. Rewrite each transaction as an intent + `client.execute`; convert take
   values to u16 and child proportions to u64 shares; replace safe-staking
   flags with `*Limit` intents.
