...
- **Queued subnet registration.** When capacity is full or cleanup is pending,
  ...
  for the matching [`NetworkAdded`](/code/pallets/subtensor/src/macros/events.rs#L13)
  by default and return the assigned `netuid`; raw-call clients must do the same
  before treating the registration as live.
