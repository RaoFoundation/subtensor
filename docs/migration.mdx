...
[V446](/releases/v446-upgrade), plus the [V448 upgrade](/releases/v448-upgrade).
...
- **Reject locked alpha.** Coldkeys that should accept locked-alpha transfers
  must opt in with `btcli conviction accept --allow` (the
  `set_reject_locked_alpha` call, also available as the
  `SetRejectLockedAlpha` intent). Default is to reject.

### Staking and fees

- **Default slippage protection.** `AddStake`, `RemoveStake`, `SwapStake`,
  and cross-subnet `MoveStake` / `MoveSwapStake` (and the matching CLI
  commands) apply a 5% price bound by default. Old any-price scripts fail
  with [`SlippageTooHigh`](/code/pallets/subtensor/src/macros/errors.rs#L192) unless you pass
  `slippage_protection=False` / `--no-slippage-protection`, or set an explicit
  `rate_tolerance` / limit price. Runtime v448 adds `move_stake_limit` for a
  single protected hotkey-and-subnet move. Its relative limit is the minimum
  acceptable destination-alpha per origin-alpha ratio. The SDK and btcli use
  fill-or-kill; direct runtime, EVM, and WASM callers can set `allow_partial`.
...
| `wallet overview` | `wallet overview` (stake portfolio) and `wallet registrations` (every hotkey UID, including zero stake) |
| `wallet inspect` / `sign` / `verify` / `encrypt` / `decrypt` | same names |
...
Spec 448 shipped two `btcli root` names that no longer exist. Do not keep
them as aliases: the later spellings do different work.

| Spec 448 command | Current command |
| --- | --- |
| `root subscribe --hotkey X --amount N` (added root stake) | `stake add --netuid 0 --hotkey X --amount N` |
| `root show --hotkey X` (one fund in detail) | `root list X` |
| `root show` with no args (pick from your positions) | `root list --mine`, then `root list X` |

Buying basket shares (β) is `root allocate`, not `subscribe`. The
network-wide fund leaderboard is bare `root list`.
