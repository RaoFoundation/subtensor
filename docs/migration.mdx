...
| `add_stake_multiple` | `bt.Batch(intents=[bt.AddStake(...), ...])` |
| `unstake_multiple` | `bt.RemoveStakes(positions=[bt.RemoveStake(...), ...])` (atomic) |
...
| `stake remove --include-hotkeys` / `--all-hotkeys` | `stake remove --hotkeys ...` / `--all-hotkeys` (atomic; `--include-hotkeys` remains an alias) |
| `stake remove --all` | `tx unstake-all` / `tx unstake-all-alpha` |
| `stake list` / `move` / `transfer` / `swap` | same names |
