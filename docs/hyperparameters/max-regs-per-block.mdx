...
- Root registration: `do_root_register`
  (`pallets/subtensor/src/coinbase/root.rs`) fails with
  `TooManyRegistrationsThisBlock` when the block's registrations reach
...
- The registration validity helper `checked_allowed_register`
...
Root/governance only: `sudo_set_max_registrations_per_block` in the
