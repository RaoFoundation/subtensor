...
while the lock is under the floor, emission is captured into the lock
instead). The release is keyed to the registered hotkey even when miner
incentive is redirected by auto-stake. At `k = 1` (the default) collateral
