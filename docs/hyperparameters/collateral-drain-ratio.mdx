...
description: Alpha of locked miner collateral released per alpha of hotkey emission earned.
---

`collateral_drain_ratio` (call it `k`) sets how fast a hotkey earns back the
...
Operators care because `lock / k` is the total emission (miner incentive and
validator dividends) they must earn before their collateral is fully
withdrawable; owners tune it to control how long hotkeys stay collateralized
— the security parameter for subnets whose scoring can be gamed on short
horizons.

## How it works

Each tempo, when hotkey emission is credited
...
optional self-maintained floor (zero unless set via `set_min_collateral`;
while the lock is under the floor, miner incentive or validator take is
captured instead). The release is keyed to the registered hotkey even when
miner incentive is redirected by auto-stake. At `k = 1` (the default) collateral
releases one-for-one with earned emission; at `k = 0.5` a hotkey must earn
two alpha per alpha released; at `k = 2` half the lock's worth of emission
releases all of it.
