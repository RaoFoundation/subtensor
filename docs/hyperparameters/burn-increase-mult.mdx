...
[`BurnIncreaseMult`](/code/pallets/subtensor/src/lib.rs#L2731) as U64F64
fixed-point (raw bits
divided by 2^64 give the real multiplier); the default is **1.26** (~3
registrations to double the price). The owner-set extrinsic
([`sudo_set_burn_increase_mult`](/code/pallets/admin-utils/src/lib.rs#L2176-L2214)
in `pallets/admin-utils/src/lib.rs`) requires
