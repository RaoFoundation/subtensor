...
The flag lives in
[`TransferToggle`](/code/pallets/subtensor/src/lib.rs#L1815) storage and
defaults to **true**. It is
checked only on the coldkey-changing path:
[`transfer-stake`](/docs/tx/transfer-stake) validates it and fails with
[`TransferDisallowed`](/code/pallets/subtensor/src/macros/errors.rs#L194)
when the toggle is off. Moving stake between hotkeys or
...
transferred. When the amount dips into locked α, the landing hotkey must match
the receiver's existing lock hotkey — see
[Transferring locked stake](/docs/guides/conviction#transferring-locked-stake).
