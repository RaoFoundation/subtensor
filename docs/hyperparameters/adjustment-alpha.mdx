...
The value lives in
[`AdjustmentAlpha`](/code/pallets/subtensor/src/lib.rs#L2117) storage as a u64
fraction where
`u64::MAX` encodes 1.0. The mainnet default is **0**
([`SubtensorInitialAdjustmentAlpha`](/code/runtime/src/lib.rs#L798) in
`runtime/src/lib.rs`) — in the legacy
...
[handled per block](/code/pallets/subtensor/src/subnets/registration.rs#L473-L515)
in `pallets/subtensor/src/subnets/registration.rs`, where
...
([`sudo_set_adjustment_alpha`](/code/pallets/admin-utils/src/lib.rs#L428-L452)
in `pallets/admin-utils/src/lib.rs`) remains
