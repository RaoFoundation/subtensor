...
Each epoch, [`alpha_sigmoid`](/code/pallets/subtensor/src/epoch/run_epoch.rs#L1538-L1565) (`pallets/subtensor/src/epoch/run_epoch.rs`) maps a pair's deviation from consensus into the EMA rate:
...
On chain, [`alpha_low`](/docs/hyperparameters/alpha-low) and `alpha_high` live in one storage value set by a single [`sudo_set_alpha_values`](/code/pallets/admin-utils/src/lib.rs#L1293-L1316) call. The CLI reads the current pair and resubmits it with only `alpha_high` changed, so the other side is preserved.
