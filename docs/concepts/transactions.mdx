...
A fee-bearing extrinsic normally pays two components in TAO from the signer's
free balance: a **weight fee**, linear in the call's dispatch weight, and a
**length fee** of 1 rao per byte of the encoded extrinsic. The estimated fee is
withdrawn before the call runs — insufficient balance rejects the transaction
outright. After execution, TAO fees are corrected for actual dispatch weight
where the call reports it; a failed call still pays its resulting fee. The
final fee and any tip are **recycled** by reducing total issuance instead of
being paid to the block author. `plan.fee` (and the `--dry-run` output) is the
chain's estimate before anything is signed.

EVM base and priority fees follow the same recycling path. For a small set of
unstake-side calls (`remove_stake` and friends), a signer with no TAO can have
the fee taken in alpha instead. The alpha is sold for TAO and the resulting TAO
is recycled atomically; alpha-paid transaction fees are final rather than
post-dispatch adjusted.
...
and take changes. Calls outside the alpha-fee fallback set require TAO.
