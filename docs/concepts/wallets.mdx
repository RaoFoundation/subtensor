...
btcli wallet overview                    # free TAO + stake per subnet
btcli wallet registrations               # every local hotkey and where it is registered
```

The same flows exist in Python under `bt.wallets` (`create`, `regen_coldkey`,
...
4. **Primary coldkey on a hardware or air-gapped signer** — a
   ...
   Polkadot generic app; [Polkadot Vault](/docs/guides/vault) signs by QR code
   from a permanently offline phone.
