...
Root-basket **beta** is not a `Balance` denomination. It is a share count in
one validator's fund — see [beta tokens](/docs/concepts/beta-tokens).
...
permanently retires supply. Transaction fees are recycled rather than paid to
the block author: native TAO and EVM fees reduce issuance directly, while
eligible alpha-paid fees are sold for TAO and recycled atomically
