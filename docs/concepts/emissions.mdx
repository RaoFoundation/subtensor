---
title: Emissions
description: TAO and alpha issuance, halvings, subnet emission shares, the per-tempo split, and Yuma Consensus.
---
...
decaying toward a hard cap of 21 million TAO. Halvings are triggered by
...
Because recycled TAO (registration burns) is subtracted from total issuance
and can be re-emitted, recycling pushes halvings out. Transaction fees are
not recycled — they are paid to the block author (see
...
halving yardstick), while circulating supply is smaller — issuance includes
...
- **`alpha_out`** — up to 1 alpha (at the subnet's current halving rate)
  destined for participants, accumulated for distribution at the next epoch.
- **`alpha_in`** — alpha injected into the pool alongside the subnet's TAO
...
Each block's TAO emission is divided across subnets in proportion to their
**EMA price**, weighted by a miner-burn penalty (this price-based formula
...
where `p_i` is the subnet's moving price (`SubnetMovingPrice`) and `b_i` is
the proportion of the last tempo's miner incentive that was withheld because
it was directed to subnet-owner hotkeys (counted whether the withheld alpha
was recycled or burned). If the combined weight is zero across all subnets,
the chain falls back to unweighted price shares so emission is never
stranded. Emission-disabled subnets get zero share, redistributed
proportionally to enabled ones — and with `tao_in` zeroed, `alpha_in`
...
claimable root dividends — but only in blocks where the sum of all subnets'
EMA prices exceeds 1.0; otherwise that alpha is recycled.
...
per block — extras are deferred one block — and a subnet owner can trigger
...
neurons by stake weight, with K = `MaxAllowedValidators` (default 128). Only
...
median: the highest weight level supported by at least `kappa` of active
...
where `alpha = 1 − bonds_moving_average / 1,000,000` (default 900,000, so
...
**Yuma3** (a per-subnet toggle) switches to fixed-point bond computation
...
per validator–miner pair, moving between `alpha_low` (default 0.7) and
...
state — [`metagraph`](/docs/query/metagraph) returns all of it in one read,
