...
V444 divides each block's TAO emission in two clear steps. First, it turns
...
`MinerBurned` does not change this cross-subnet share in v444. It still records
what happened to miner incentive inside the subnet, but one subnet's local burn
or recycle choice no longer changes another subnet's emission.
