...
- **Registration** — neuron registration calls ([`burned_register`](/code/pallets/subtensor/src/macros/dispatches.rs#L826-L832),
  [`register`](/code/pallets/subtensor/src/macros/dispatches.rs#L804-L814), [`register_limit`](/code/pallets/subtensor/src/macros/dispatches.rs#L2242-L2249)).
...
- **BasketTrading** — `swap_basket` only.
- **Validate** (spec 470+) — validator hotkey operations: all weights calls,
  `serve_axon` / `serve_axon_tls` / `serve_prometheus`, `associate_evm_key`,
  and `Commitments.set_commitment`. Nothing that moves stake or TAO.
- **Weights** (spec 470+) — the weights calls only; a strict subset of
...
pallet's [`EncryptedSubmitted`](/code/pallets/shield/src/lib.rs#L206) event, and a successful submission carries
...
are set. Vault, Ledger, and the browser extension each sign that inner
call and then the carrier — two approvals, same 8-block era. The
extension bridge labels them "1 of 2" and "2 of 2".
...
enables the pallet. [`execute_batched_orders`](/code/pallets/limit-orders/src/lib.rs#L469-L481) nets a batch's buys against its
...
`bt.calls.LimitOrders.*` ([`execute_orders`](/code/pallets/limit-orders/src/lib.rs#L419-L443), `execute_batched_orders`,
[`cancel_order`](/code/pallets/limit-orders/src/lib.rs#L490-L511), [`set_pallet_status`](/code/pallets/limit-orders/src/lib.rs#L520-L538)).
