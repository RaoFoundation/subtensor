...
| 13 | `add_proxy` | Add a staking proxy for the contract's account | `(AccountId)` | Error code |
| 14 | `remove_proxy` | Remove a staking proxy for the contract's account | `(AccountId)` | Error code |
...
<Callout type="warning">
From spec version **456**, `Caller*` IDs **20–33** and **38** are disabled and reserved; calls trap with `DispatchError::BadOrigin` rather than returning a status code. State-changing extensions use the **contract's own account**, never its caller's authority.
</Callout>
