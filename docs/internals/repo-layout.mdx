...
| `py-sp-core/` | PyO3 bindings around `sp-core` (keypairs, signing) — the native backbone of the Python SDK, published as wheels |
| `bittensor-drand/` | PyO3 bindings for drand timelock encryption (commit-reveal weights, general commitments) — lives here so its epoch-schedule simulation moves in lockstep with `pallets/subtensor`; published as wheels |
