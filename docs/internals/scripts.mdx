...
| `srtool/build-srtool-image.sh` | `propose-mainnet-upgrade.yml` — builds the local srtool Docker image |
...
| Script | Purpose |
|--------|---------|
| `code-coverage.sh` | Coverage run described in the root README (requires `cargo-tarpaulin`) |
...
the wasm hash CI proposed on-chain (via `propose-mainnet-upgrade.yml`, which
