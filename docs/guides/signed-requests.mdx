...
MY_HOTKEY = bt.Wallet(name="my_coldkey", hotkey="my_hotkey").hotkey.ss58_address

app = FastAPI()

async def authenticated(request: Request) -> bt.http_auth.Caller:
...
@app.post("/generate")
async def generate(request: Request, caller: bt.http_auth.Caller = Depends(authenticated)):
...
def allowed(caller: bt.http_auth.Caller) -> bool:
    n = mg.by_hotkey(caller.hotkey_ss58)
    return n is not None and n.validator_permit and n.tao_stake >= bt.tao(1000)
```
...
class HotkeyAuth(httpx.Auth):
    ...
    def auth_flow(self, request):
        request.headers.update(bt.http_auth.sign(
            self.wallet,
            method=request.method,
...
async def query_miner(wallet, miner: bt.MetagraphNeuron, body: bytes):
...
The Python `bt.http_auth.build_payload(...)` function produces these payload
