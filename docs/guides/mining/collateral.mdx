...
description: Lock a share of the registration price as a bond you earn back by mining; misbehave and it stays frozen.
---
...
### Voluntary collateral and the self-maintaining floor

Beyond the registration lock, two miner-initiated extrinsics let a subnet run
...
- [`add_collateral`](/code/pallets/subtensor/src/subnets/collateral.rs#L235-L292)
  stakes TAO to your own hotkey and locks it on top of the registration
  collateral. Same lock, same drain, same persistence across deregistration
  and credit on re-registration.
- [`set_min_collateral`](/code/pallets/subtensor/src/subnets/collateral.rs#L299-L342)
  sets a **floor** the lock self-maintains around: the drain never releases
  below it, and while the lock is under it, your earned incentive is captured
  into the lock until the floor is met. Without a floor, tracking a
  validator-published requirement means re-locking whatever the drain
  releases every tempo; with one, the chain does it for you. Zero clears the
  floor.

```bash
btcli collateral add --netuid 42 --amount-tao 50 -w my_coldkey -H my_hotkey
btcli collateral set-min --netuid 42 --min-alpha 45 -w my_coldkey -H my_hotkey
btcli collateral show --netuid 42 -H my_hotkey     # your lock, floor, and the subnet policy
btcli collateral list --netuid 42                  # every miner's collateral (what validators read)
```

```python
await client.execute(bt.AddCollateral(netuid=42, amount_tao=50), wallet)
await client.execute(bt.SetMinCollateral(netuid=42, min_alpha=45), wallet)
state = await client.collateral.miner_collateral(netuid=42, hotkey_ss58=wallet.hotkey_address)
```

The floor is per-miner state, not a subnet hyperparameter: adequacy
enforcement stays with validators, who read
[`subnet_collateral`](/docs/query/subnet-collateral) and refuse to score
miners whose lock is below the subnet's published requirement. Raising the
floor above your current lock needs no fresh capital — the shortfall fills
from future incentive — but validators see your actual lock, not your floor,
so fund immediately with `add_collateral` when coverage matters now.

The full pattern — owner config, miner flow, validator enforcement code, and
the exit protocol — is worked through in
[a GPU marketplace with per-machine deposits](#a-gpu-marketplace-with-per-machine-deposits)
below.

The whole lifecycle in one picture — drag the sliders to see how the lock
share, drain ratio, and your earning rate shape the bond, and switch to the
blacklisted scenario to see what stranding looks like:
...
registration price including a fresh full lock. The system's invariant is
exactly that: *forgetting is never cheaper than `T`.*
...
### A subnet deters tail-risk farming
...
### A GPU marketplace with per-machine deposits

The Lium pattern: miners bring machines, each machine must be backed by a
deposit, pulling a rented machine forfeits it, and an honest departure winds
down cleanly. The chain provides amount-agnostic rails; the *per-machine*
policy lives in validator code.
...
**Miner flow** — bring 4 machines, deposit 100α, let the floor hold it:
...
The floor stops the drain at 100α, and if the lock ever dips under (say the
miner raises the floor for a fifth machine before funding it), incentive
fills the gap automatically. No re-locking, no bot.
...
- **earned** — lifetime incentive since the collateral relationship began;
  compare against the detection budget `E* = T ÷ (1 + k)` to see how far a
  miner is from having "paid off" their bond. Lives and dies with the entry.
