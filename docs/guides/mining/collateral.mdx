...
The chain never adjudicates miner behavior — there is no ban extrinsic.
Validators score miners off-chain, and a validator that concludes a hotkey is
cheating simply **stops evaluating it**. That cuts miner incentive. A miner-only
hotkey with no other emission stops releasing collateral; a validator hotkey
may continue draining through dividends.

- It is **stake-weighted by construction**: one validator refusing to score
  you reduces miner incentive by its weight in consensus; a supermajority
  blacklist cuts it to zero.
...
[The V437 Upgrade](/releases/v436-upgrade). The system's invariant is exactly
...
machine**. Sizing the deposit in emission terms matters: if one machine
...
a fifth machine before funding it), miner incentive or validator take fills
the gap automatically. No re-locking, no bot.
...
`collateral_min` (the miner's floor), and `collateral_earned` (lifetime hotkey
emission since the bond existed):
