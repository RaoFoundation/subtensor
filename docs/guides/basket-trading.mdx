...
**Basket trading** (runtime v460) lets a root validator actively manage the
...
For what changed on chain in v460, see the
[release notes](/releases/v460-upgrade). Two companion pages cover the other
...
proxy. Runtime v460 adds a proxy type for exactly this:
...
btcli quotes the fund and the trade first, then shows a review card before
...
btcli root swap --from 8 --to 64 --amount 250 --max-slippage 0.5 ...  # tighter floor: fill within 0.5% of the quote
```
...
- Release: [v460 — Basket Trading](/releases/v460-upgrade)
