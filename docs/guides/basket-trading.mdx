...
`BasketConcentrationCap` of fund NAV — default 1/16, so a traded fund
spreads across at least 16 holdings. Topping up a holding that is already at
the cap is refused, and so is moving more than 1/16 of NAV into the cash
slot. Only buys are checked: a holding that grows past the cap through price
moves or in-place dividends is left alone. On a chain with fewer than 16
subnets the check is skipped. Refusal is `BasketConcentrationCapExceeded`.
