...
btcli root swap --from 8 --to 64 --amount 250 --hotkey 5F...val \
  --max-slippage 1 -w desk --proxy-for 5F...validator_coldkey -n test --dry-run
```

`--dry-run` shows the review card and the fee without submitting; drop it
to sign and send. btcli quotes the fund and the trade first, then shows a review card before
...
frozen). Confirm, and the call is proxied, shielded, and submitted.
...
```bash
btcli root swap --from 3 --to 0 --amount all --hotkey 5F...val -n test ...   # exit netuid 3 to cash
btcli root swap --from 0 --to 21 --amount 500 --hotkey 5F...val -n test ...  # spend 500 τ of cash on netuid 21
btcli root swap --from 8 --to 64 --amount 250 --max-slippage 0.5 -n test ...  # tighter floor: fill within 0.5% of the quote
```
