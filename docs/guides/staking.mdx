...
If you build calls yourself, do not size a follow-up from a `StakeAdded` event:
a position is stored as pool shares and reads back truncated, so the alpha the
event reports can be one rao above what the position returns, and a
`transfer_stake` for that figure fails with `NotEnoughStakeToWithdraw`. Pass
`alpha_amount = u64::MAX` instead: since spec 469 every debit call
(`move_stake*`, `transfer_stake*`, `swap_stake*`) reads it as "the whole live
position at execution" and leaves no dust behind.
...
built from the dividends it earns (held on the subnets they were earned on and
reshaped only by the validator's `swap_basket` trades), and root stakers accrue
an entitlement to that fund in proportion to their root stake.
...
The claim fee scales with how many ALPHA types are in the basket. The chain
reserves a 129-unit envelope for a single-validator claim and the full 256-unit
envelope for a coldkey-wide claim, then refunds the unused part after. Admission
...
validator-side trading commands, and migration notes from the retired
