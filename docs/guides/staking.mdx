...
`btcli` shows root positions in **beta (β)**: staked β is principal on
netuid 0, accrued β is fund yield. List your positions, then redeem accrued
β — one parameterless claim covers every validator you root-stake to,
paying out in TAO staked back to root (unstaking past your staked β also
merges accrued β automatically):

```bash
btcli root list                    # staked β + accrued β, per validator
btcli tx claim-root -w my_coldkey  # redeem via claim-root
```
