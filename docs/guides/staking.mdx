...
To exit several positions, keep using `stake remove` and select hotkeys with
the ordinary staking flags. The amount and slippage settings apply to every
selected position, and the removals execute atomically:

```bash
btcli stake remove \
  --include-hotkeys validator-a,validator-b \
  --amount-alpha all \
  --rate-tolerance 0.1 \
  --dry-run
```

Omitting `--netuid` selects matching positions on every subnet. Pass
`--netuid 1` to restrict the batch to one subnet. Use `--all-hotkeys` to select
every non-zero position, optionally with
`--exclude-hotkeys validator-c,validator-d`.

For different amounts or slippage settings per position, compose ordinary
`RemoveStake` intents with the canonical [`Batch`](/docs/tx/batch) intent:
...
This replaces the SDK's pre-v11 `unstake_multiple` helper. Unlike the old
sequence of independent transactions, `Batch` is all-or-nothing. Positions on
the same netuid share one subnet pool and execute in list order, so earlier
removals can move the price seen by later ones.
