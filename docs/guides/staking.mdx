...
- Setting or revoking children is rate-limited to once per 150 blocks
  ...
  [`pending-children`](/docs/query/pending-children). The cooldown is skipped
  when a reassignment only redirects allocation freed from children that are
  unregistered, no longer validator-permitted, or inactive on every subnet
  mechanism. Reducing an active child's allocation keeps the normal cooldown.
