...
localnet has no EVM chain ID and blocks contract deployment behind a
whitelist — `btcli evm setup-localnet -w alice` fixes both in one command
(`--chain-id 945` by default).
