...
description: The metagraph, alpha, uid-lookup, and balance precompiles — free view calls from the CLI, Python, and Solidity.
---
...
`balance` (address `0x…080e`) reads any native ss58 account's free TAO
balance. Like other key-shaped precompile arguments, Solidity passes the raw
32-byte public key and the CLI accepts ss58 directly:

```bash
btcli evm call balance getFreeBalance 5F...
```

The return value is rao.
