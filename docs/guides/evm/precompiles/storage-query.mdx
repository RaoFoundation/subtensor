...
| `Scheduler` | Add the proposed [Scheduler](./scheduler) precompile. |
| `Drand` | Add the proposed [Drand](./drand) precompile. |
...
| `Timestamp` | Add the proposed [Timestamp](./timestamp) precompile for complete typed coverage; `getTimestamp()` is equivalent to the existing EVM `block.timestamp` value. |
