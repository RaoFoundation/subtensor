...
transfer when a single hotkey accrues enough conviction of its own.
...
| Ownership threshold | 1,080,000 α | 18% of eligible alpha — one hotkey must exceed this alone |
...
| Bob (validator) | 1,200,000 α locked | Perpetual lock toward his validator hotkey |
| Carol (staker) | 200,000 α locked | Decaying lock (default) toward a validator |

<ConvictionSubnetScenario />

Drag **elapsed time** forward: Bob's perpetual conviction climbs toward 1.2M α.
Carol's decaying conviction **rises then falls** as her locked mass frees.
When **Bob's own conviction** crosses 1.08M α and the subnet is old enough, his
hotkey becomes the new owner. Alice's and Carol's conviction never counts
toward Bob's gate — only locks targeted at his hotkey do.
...
After each epoch, the chain runs [`change_subnet_owner_if_needed`](/code/pallets/subtensor/src/staking/lock.rs#L1174-L1398) when **all** of
these hold:

1. Subnet age ≥ **ONE_YEAR** (2,629,800 blocks from registration).
2. The hotkey with the highest rolled aggregate conviction holds **more than
   18%** of eligible alpha on its own:
   ...
   zero. Only that one hotkey's conviction counts; the owner's locks and
   other keys' locks do not add to it.
3. That hotkey resolves to a real coldkey owner (not the default account).
...
Measuring the winning hotkey alone, rather than the subnet-wide total, is
deliberate. Admission is decided on one hotkey and the winner is the maximum,
so gating on the sum let every unrelated locker (the incumbent owner included)
supply a challenger's quorum. Coalitions still work: backers lock to the
challenger's hotkey, so their conviction lands in that hotkey's aggregate.

The per-hotkey projection in
[`subnet-convictions`](/docs/query/subnet-convictions) estimates when each
hotkey might reach 18% of eligible alpha on its own — the same bar the chain
applies. Treat it as a heuristic: it assumes lock rates and alpha accounting
stay constant.
...
Recipient coldkeys reject locked α by default — they must opt in with
`btcli conviction accept --allow` (the `set_reject_locked_alpha` call) before
accepting a transfer that moves lock mass
([`AccountRejectsLockedAlpha`](/docs/errors/chain/AccountRejectsLockedAlpha)).
`btcli conviction list` shows the current setting; `--reject` restores the
default.
