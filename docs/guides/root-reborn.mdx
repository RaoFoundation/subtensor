...
- **Only trades move composition.** Since runtime v460 a validator reshapes
...
the accumulate-in-place strategy, and runtime v460 made that the only
...
```bash
btcli root list 5F...                                 # holdings, NAV, lifetime return
btcli root swap --hotkey 5F... --from 4 --to 8 --amount 100   # sell 100 α of subnet 4, buy subnet 8
btcli root swap --hotkey 5F... --from 0 --to 8 --amount 50    # deploy 50 τ of cash into subnet 8
btcli root swap --hotkey 5F... --from 8 --to 0 --amount all   # take profit into cash
```
...
Runtime v460 removed it again. Weights only ever decided how an
...
deposits harder to reason about, so v460 made accumulate-in-place the only
