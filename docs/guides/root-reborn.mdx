...
- **Claims are proportional — or paid from cash.** A claim redeems the staker's
  owed fraction of every holding pro-rata, preserving fund composition. Since
  spec 468 a fund that holds TAO in its cash slot (netuid 0) pays claims from
  that cash first: the staker's share is valued at the guarded mark (each
  holding at the lower of its realizable quote and its fast-moving-price value)
  and paid in TAO with no sale, up to the cash on hand and the fund's daily
  cash-claim budget (`BasketCashClaimCap`). The path ships **off** (cap `0`) and
  governance opens it (`655` = the sized 1% of guarded NAV per day). What the
  cash does not cover stays owed and redeems pro-rata on the next claim.
