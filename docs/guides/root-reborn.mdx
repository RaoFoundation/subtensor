...
toward the floor (τ0.0005) with a ~72-minute half-life, so entry is nearly
free when the network is quiet and expensive to spam.

```bash
btcli subnets burn-cost 0                        # current root registration price
btcli subnets register --netuid 0 --dry-run      # preview fee and effects
btcli subnets register --netuid 0 -w my_coldkey  # register the wallet hotkey
```
...
Redeeming more than your staked τ claims accrued yield into staked principal
first (a claim under the hood), so you never have to claim manually just to
exit. To realize accrued yield explicitly — compounding it into your staked
principal — submit the claim directly:
...
validator you root-stake to, redeems your accrued entitlement as a pro-rata slice of
...
- **Your validator is now a fund manager.** Where your yield goes is their
  ...
  redeem and subscribe to a different validator; your principal moves at face
  value.
...
- **Unstaking can have a hold window.** If the network sets
  `RootStakeUnlockInterval`, root stake is locked for that many blocks after
  your last stake change (anti-sniping around epoch boundaries). A redeem
  inside the window fails with `RootStakeLocked` — wait it out and retry.
