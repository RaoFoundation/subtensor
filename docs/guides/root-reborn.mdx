...
- **Claims are proportional, minus dust.** A claim redeems the staker's owed
  ...
  prices against — is neither sold nor paid. The claim still settles your
  whole entitlement, so those slices (each under 0.0001 TAO) stay in the fund
  for the remaining holders; the `BasketClaimDustSkipped` event reports how
  many rows and roughly how much. On finney's claim flow this removes about a
