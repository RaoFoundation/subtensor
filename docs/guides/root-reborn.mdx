...
- **Claims are proportional, minus dust.** A claim redeems the staker's owed
  ...
  less than 0.001 TAO — both at the fast-anchored mark `swap-basket` already
  prices against — is neither sold nor paid. Nothing is lost:
  the claim burns only the shares that match what it redeemed, so you keep
  the shares for the skipped slices and they pay out in a later, larger
  claim (the `BasketClaimDustSkipped` event shows how many shares and roughly
  what they are worth). This is what keeps a small claim from paying a swap
  per row for amounts that round to nothing; governance can tune or zero the
  floors with `sudo_set_basket_claim_dust`.
...
- **Dust self-consolidates, and fees follow real work.** Any basket holding
  ...
  expensive forever. A claim that fails is charged for the work it did, not
