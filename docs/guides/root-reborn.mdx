...
`btcli root claim`. Per-validator breakdown is `btcli root list`.
...
free balance — accrued yield is claimed first when needed. Under the hood that
uses [`claim-root-with-hotkey`](/docs/tx/claim-root-with-hotkey) (and
