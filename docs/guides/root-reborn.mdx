...
- **Claims are proportional, minus dust.** A claim redeems the staker's owed
  fraction of every holding pro-rata, preserving fund composition. Since
  spec 468 it leaves dust rows alone: a holding worth less than
  `min(1 TAO, 0.1% of the fund's NAV)`, or one where your own slice is worth
  less than 0.0001 TAO — both at the fast-anchored mark `swap-basket` already
  prices against — is neither sold nor paid, provided your slice of it is at
  most 0.01 TAO (a bigger slice of a small row is sold as before). The claim
  still settles your whole entitlement, so those slices — never more than
  0.01 TAO each at the anchored mark — stay in the fund for the remaining
  holders; the `BasketClaimDustSkipped` event reports how many rows and
  roughly how much. On finney's claim flow this removes about a
  fifth of the swaps while a median claim leaves under 1% of its value
  behind. Governance can tune or zero the floors with
...
broken `btcli`. On testnet also pass `--no-mev-shield`: the MEV shield's
validator-side reveal runs on mainnet only, so a shielded swap on testnet
is accepted but never executes. On mainnet leave that flag off; swaps
shield by default. See
...
- **Claim fee.** The inclusion fee scales with how many ALPHA types the
  basket holds. The chain reserves a 129-unit envelope for a single-validator
  claim and the full 256-unit envelope for a coldkey-wide claim, then refunds
  the unused part after. Admission
...
- **Dust self-consolidates, and fees follow real work.** Any basket holding
  ...
  consolidated holdings pay full weight, holdings merely scanned (including
  dust rows a claim skips) pay a small per-row cost — so a fund littered with
  dust positions gets cheaper to claim after its first claim, not more
  expensive forever. A claim that is admitted and then fails is charged for
  the work it did, not the declared envelope — and since spec 469 so is every
  other heavy call (stake exits, `unstake_all`, basket deposits and trades,
  key swaps, registration, lease termination), for TAO and alpha payers alike.
