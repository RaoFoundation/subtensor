...
- **Default is 100% TAO (the root slot).** A validator with no custom
  weights still accrues — dividends land in the fund's root cash slot as
  plain TAO, with no forced subnet exposure and a single holding to claim
  against. Deploying into subnets is an explicit act of curation:
  `set_root_weights` requires at least 8 positive destinations (softened
  when fewer networks exist), and netuid 0 remains a valid destination to
  keep part of a curated basket in TAO.
