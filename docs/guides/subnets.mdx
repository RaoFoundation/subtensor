...
[`set-weights`](/docs/tx/set-weights). Read the current count with
[`mechanism-count`](/docs/query/mechanism-count).
...
- Count and split changes are each rate-limited to once per 7,200 blocks
  (~24 hours).
