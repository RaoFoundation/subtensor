...
# Full docs corpus (every page in one file)
...
# Chain source: list paths, then fetch a file
curl -s https://www.bittensor.com/code/index.json
curl -s https://www.bittensor.com/code/raw/pallets/subtensor/src/lib.rs | rg -n '<pattern>'

# Heavy exploration: clone the repo (docs live in docs/)
git clone --depth 1 https://github.com/RaoFoundation/subtensor
```

Prefer [`/llms.txt`](/llms.txt) plus per-page `content.md` and the JSON
catalogs below over dumping [`/llms-full.txt`](/llms-full.txt) into context.
`llms-full.txt` is for offline search and RAG, not for loading whole into a
session.
...
`docs_url` pointing at its reference page.
