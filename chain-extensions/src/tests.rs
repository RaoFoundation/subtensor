...
#[test]
fn caller_endpoints_reject_before_input_decoding() {
    mock::new_test_ext(1).execute_with(|| {
        for func_id in [
            ...
            // Rejection must precede decoding the missing input.
            let mut env = MockEnv::new(func_id, U256::from(1), Vec::new());
            assert_eq!(
                SubtensorChainExtension::<mock::Test>::dispatch(&mut env).map(|_| ()),
                Err(DispatchError::BadOrigin),
                "function ID {} must remain disabled",
                env.func_id,
            );
        }
    });
...
#[test]
fn legacy_wasm_cannot_spend_caller_assets_or_grant_proxy_authority() {
    use pallet_contracts::{Code, CollectEvents, DebugInfo, Determinism};

    mock::new_test_ext(1).execute_with(|| {
        let victim = U256::from(90001);
        let hotkey = U256::from(90002);
        let attacker = U256::from(90003);
        let amount = DefaultMinStake::<mock::Test>::get().saturating_mul(100.into());
        let netuid = mock::add_dynamic_network(&hotkey, &victim);
        mock::setup_reserves(
            netuid,
            amount.saturating_mul(20.into()),
            AlphaBalance::from(amount.to_u64().saturating_mul(30)),
        );
        for account in [victim, attacker] {
            mock::add_balance_to_coldkey_account(
                &account,
                amount
                    .saturating_mul(100.into())
                    .saturating_add(1_000_000_000.into()),
            );
        }
        assert_ok!(pallet_subtensor::Pallet::<mock::Test>::add_stake(
            RawOrigin::Signed(victim).into(),
            hotkey,
            netuid,
            amount,
        ));

        // Legacy fixture; selectors are from ts-tests/ink/bittensor.json.
        let gas = Weight::from_parts(100_000_000_000, 10_000_000);
        let instance = mock::Contracts::bare_instantiate(
            attacker,
            TaoBalance::ZERO,
            gas,
            None,
            Code::Upload(include_bytes!("../../ts-tests/ink/bittensor.wasm").to_vec()),
            vec![0x9b, 0xae, 0x9d, 0x5e], // new
            Vec::new(),
            DebugInfo::Skip,
            CollectEvents::Skip,
        )
        .result
        .unwrap();
        assert!(!instance.result.did_revert());
        let contract = instance.account_id;
        mock::add_balance_to_coldkey_account(&contract, amount.saturating_mul(2.into()));
        assert_ne!(contract, victim);
        let stake_of = |coldkey: &U256| {
            pallet_subtensor::Pallet::<mock::Test>::get_stake_for_hotkey_and_coldkey_on_subnet(
                &hotkey, coldkey, netuid,
            )
        };
        let victim_stake = stake_of(&victim);
        let victim_balance = pallet_subtensor::Pallet::<mock::Test>::get_coldkey_balance(&victim);
        let victim_proxies = pallet_subtensor_proxy::Proxies::<mock::Test>::get(victim);

        for (selector, input) in [
            ([0xa5, 0xb8, 0xd0, 0x94], (hotkey, netuid, amount).encode()), // caller_add_stake
            (
                [0x7e, 0xf3, 0xa2, 0x8d], // caller_transfer_stake
                (attacker, hotkey, netuid, netuid, victim_stake).encode(),
            ),
            ([0xe6, 0xa2, 0x02, 0x39], attacker.encode()), // caller_add_proxy
        ] {
            let mut data = selector.to_vec();
            data.extend(input);
            let result = mock::Contracts::bare_call(
                victim,
                contract,
                TaoBalance::ZERO,
                gas,
            ...
            assert_eq!(result.result.map(|_| ()), Err(DispatchError::BadOrigin));
            assert_eq!(stake_of(&victim), victim_stake);
            assert_eq!(stake_of(&attacker), AlphaBalance::ZERO);
            assert_eq!(
                pallet_subtensor::Pallet::<mock::Test>::get_coldkey_balance(&victim),
                victim_balance,
            );
            assert_eq!(
                pallet_subtensor_proxy::Proxies::<mock::Test>::get(victim),
                victim_proxies
            );
        }

        // Ordinary calls must still spend only the contract's funds.
        let mut data = vec![0x3a, 0x65, 0x6e, 0x31]; // add_stake
        data.extend((hotkey, netuid, amount).encode());
        let result = mock::Contracts::bare_call(
            victim,
            contract,
            TaoBalance::ZERO,
        ...
        assert!(stake_of(&contract) > AlphaBalance::ZERO);
        assert_eq!(stake_of(&victim), victim_stake);
        assert_eq!(
            pallet_subtensor::Pallet::<mock::Test>::get_coldkey_balance(&victim),
            victim_balance,
