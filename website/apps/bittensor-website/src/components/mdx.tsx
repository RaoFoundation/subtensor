...
import {LiumCollateralExample} from './docs/collateral-lium-example';
import {TradingCollateralExample} from './docs/collateral-trading-example';
...
import {BetaFundDiagram} from './docs/beta-fund-diagram';
...
export function Cards({children}: {children: ReactNode}) {
  return (
    <div className='grid gap-px sm:grid-cols-2 border border-line bg-line not-prose my-6'>
      {children}
    </div>
  );
}
...
export function Card({
  ...
  return (
    <Link href={href} className={cn('block bg-bg p-5 transition-colors hover:bg-panel')}>
      <p className='bt-label mb-2'>{title}</p>
      {description && <p className='text-[0.8125rem] leading-relaxed text-mute'>{description}</p>}
    </Link>
  );
}
