...
// A hotkey swap moves every stake position of the hotkey and is measured for a bounded number
// of them. A hotkey above the bound is refused before any write; swapping one subnet at a
// time (whose rows fit) still works, so large hotkeys can rotate subnet by subnet.
#[test]
fn test_swap_hotkey_refuses_more_positions_than_priced() {
    new_test_ext(1).execute_with(|| {
        let subnet_owner_coldkey = U256::from(1001);
        let subnet_owner_hotkey = U256::from(1002);
        ...
        SubtensorModule::create_account_if_non_existent(&owner, &old_hotkey);
        add_balance_to_coldkey_account(&owner, TaoBalance::from(1_000_000_000_000_u64));

        // More positions than one call is priced for, spread over two subnets so that a
        // single subnet stays within the bound.
        let per_subnet = (MAX_HOTKEY_SWAP_POSITIONS / 2) as u64 + 1;
        for i in 0..per_subnet {
            let nominator = U256::from(10_000 + i);
        ...
        assert!(
            SubtensorModule::alpha_iter_single_prefix(&old_hotkey).count()
                > MAX_HOTKEY_SWAP_POSITIONS as usize
        );
        ...
        assert_err!(
            ...
            Error::<Test>::HotkeySwapTooHeavy
        );
        // Nothing moved.
        assert!(TotalHotkeyAlpha::<Test>::get(new_hotkey, netuid_a).is_zero());

        // One subnet at a time fits.
        let alpha_a_before = TotalHotkeyAlpha::<Test>::get(old_hotkey, netuid_a);
        ...
        assert_eq!(
            TotalHotkeyAlpha::<Test>::get(new_hotkey, netuid_a),
            alpha_a_before
        );
        assert!(TotalHotkeyAlpha::<Test>::get(old_hotkey, netuid_a).is_zero());
    });
}
