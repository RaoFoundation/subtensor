...
#[test]
fn test_migrate_subnet_limit_256_raises_below_target() {
    use crate::migrations::migrate_subnet_limit_256::{
        self, MIGRATION_NAME, TARGET, post_migrate_subnet_limit_256, pre_migrate_subnet_limit_256,
    };

    new_test_ext(1).execute_with(|| {
        HasMigrationRun::<Test>::remove(MIGRATION_NAME.to_vec());
        SubnetLimit::<Test>::put(128);

        let pre = pre_migrate_subnet_limit_256::<Test>();
        assert_eq!(pre.previous_limit, 128);
        assert!(!pre.already_run);

        let w = migrate_subnet_limit_256::migrate_subnet_limit_256::<Test>();
        assert!(!w.is_zero());

        assert_eq!(SubnetLimit::<Test>::get(), TARGET);
        assert!(HasMigrationRun::<Test>::get(MIGRATION_NAME.to_vec()));
        post_migrate_subnet_limit_256::<Test>(pre).expect("post-migrate checks");
    });
}

#[test]
fn test_migrate_subnet_limit_256_preserves_above_target() {
    use crate::migrations::migrate_subnet_limit_256::{
        self, MIGRATION_NAME, post_migrate_subnet_limit_256, pre_migrate_subnet_limit_256,
    };

    new_test_ext(1).execute_with(|| {
        HasMigrationRun::<Test>::remove(MIGRATION_NAME.to_vec());
        let above: u16 = 300;
        SubnetLimit::<Test>::put(above);

        let pre = pre_migrate_subnet_limit_256::<Test>();
        let w = migrate_subnet_limit_256::migrate_subnet_limit_256::<Test>();
        assert!(!w.is_zero());

        assert_eq!(SubnetLimit::<Test>::get(), above);
        assert!(HasMigrationRun::<Test>::get(MIGRATION_NAME.to_vec()));
        post_migrate_subnet_limit_256::<Test>(pre).expect("post-migrate checks");
    });
}
...
#[test]
fn test_migrate_subnet_limit_256_repeated_is_noop() {
    use crate::migrations::migrate_subnet_limit_256::{
        self, MIGRATION_NAME, TARGET, post_migrate_subnet_limit_256, pre_migrate_subnet_limit_256,
    };
    ...
    new_test_ext(1).execute_with(|| {
        HasMigrationRun::<Test>::remove(MIGRATION_NAME.to_vec());
        SubnetLimit::<Test>::put(64);

        // First run raises to 256.
        let first_pre = pre_migrate_subnet_limit_256::<Test>();
        migrate_subnet_limit_256::migrate_subnet_limit_256::<Test>();
        post_migrate_subnet_limit_256::<Test>(first_pre).expect("first post-migrate");
        assert_eq!(SubnetLimit::<Test>::get(), TARGET);

        // Second run must leave the limit and flag untouched.
        let second_pre = pre_migrate_subnet_limit_256::<Test>();
        assert!(second_pre.already_run);
        assert_eq!(second_pre.previous_limit, TARGET);

        let w = migrate_subnet_limit_256::migrate_subnet_limit_256::<Test>();
        assert!(!w.is_zero(), "noop path still charges the HasMigrationRun read");

        assert_eq!(SubnetLimit::<Test>::get(), TARGET);
        assert!(HasMigrationRun::<Test>::get(MIGRATION_NAME.to_vec()));
        post_migrate_subnet_limit_256::<Test>(second_pre).expect("noop post-migrate");
    });
}
