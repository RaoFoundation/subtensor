...
use crate::{
    BasketClaimed, BasketRate, BasketShares, DefaultMinRootClaimAmount, DefaultMinStake, Error,
...
// =============================================================================
// Beta basket: direct deposits (`stake_into_basket`), ΔNAV minting, and
// root-slot yield attribution
// =============================================================================
...
fn test_stake_into_basket_empty_fund_par_mint_equals_nav() {
    new_test_ext(1).execute_with(|| {
        ...
        assert!(shares >= amount * 95 / 100, "entry cost should be fees-only");
    });
}
...
fn test_stake_into_basket_does_not_dilute_existing_holders() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_abs_diff_eq!(
            alice_payout_mid,
            alice_payout_before,
            epsilon = alice_payout_before / 50
        );
        assert_abs_diff_eq!(share_price(&hotkey), price_before, epsilon = 0.02);
        assert_shares_fully_owed(&hotkey, &[alice, bob], 2);
...
fn test_stake_into_basket_credit_survives_stake_changes() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_shares_fully_owed(&hotkey, &[alice, bob], 3);
    });
}
...
fn test_basket_deposit_mints_delta_nav_on_thin_pool() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_abs_diff_eq!(payout, nav, epsilon = nav / 100);
    });
}
...
fn test_root_slot_yield_accrues_to_share_holders() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_abs_diff_eq!(minted2, expected_minted, epsilon = expected_minted / 100 + 2);
        assert!(
            minted2 < delta2,
        ...
        let nav = SubtensorModule::get_validator_basket_nav_tao(&hotkey).to_u64();
        assert_abs_diff_eq!(alice_payout + bob_payout_after, nav, epsilon = 3);
    });
}
...
fn test_stake_into_basket_cannot_skim_compounding() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_abs_diff_eq!(bob_payout, amount, epsilon = amount / 20);
