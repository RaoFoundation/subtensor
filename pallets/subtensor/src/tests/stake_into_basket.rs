...
/// Spec 469: a fully drained holding (alpha with zero realizable value) is left out of the
/// mirror instead of blocking the deposit. It contributes nothing to the NAV the shares are
/// priced at, so the new shares claim nothing of it; the next dust sweep writes it off. A
/// barely priceable holding whose deposit slice rounds to zero still rejects the deposit.
#[test]
