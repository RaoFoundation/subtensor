...
impl From<&LockState> for LockContribution {
...
impl ConvictionModel {
    ...
    /// Adds locked mass while keeping the individual and aggregate
    ...
    fn add_locked_mass(
        ...
        let mut lock = self.detach_individual();
        lock.locked_mass = lock.locked_mass.saturating_add(amount);
        lock = Self::roll_forward_lock(
            lock,
            now,
            unlock_rate,
            maturity_rate,
    ...
    /// Returns the individual lock rolled to `now` without changing model
    /// state or dirty flags.
    ...
    /// Removes the complete canonical individual contribution from this model
    /// and its aggregate bucket.
    fn detach_individual(&mut self) -> LockState {
        let detached = self.individual_lock.clone();
        let contribution = LockContribution::from(&detached);
        self.individual_lock = Self::reduce_lock(
            &self.individual_lock,
            contribution.locked_mass,
            contribution.conviction,
        );
        self.individual_lock_dirty = true;
        self.reduce_aggregate(contribution);
        detached
    }

    /// Adds one canonical individual contribution after the previous
    /// contribution has been detached from this model and its aggregate.
    fn attach_to_detached_individual(&mut self, lock: LockState) {
        let contribution = LockContribution::from(&lock);
        self.individual_lock = lock;
        self.individual_lock_dirty = true;
        self.add_to_aggregate(contribution);
    }

    /// Rolls the individual lock and its aggregate bucket forward together.
    ///
    /// Individual locks and their aggregate can have different timestamps
    /// because aggregate-only operations may advance the bucket without
    /// rewriting every member. Before rolling to `now`, reconstruct the
    /// individual's contribution at a common anchor, detach it from the
    /// aggregate, roll the remaining members and the individual separately,
    ...
    pub fn roll_forward(&mut self, now: u64, unlock_rate: u64, maturity_rate: u64) {
        ...
        let individual_at_anchor = Self::roll_forward_lock_preserving_dust(
            self.individual_lock.clone(),
            anchor,
            unlock_rate,
            maturity_rate,
        ...
        let aggregate_at_anchor = Self::roll_forward_lock(
            self.aggregate_lock().clone(),
            anchor,
            unlock_rate,
            maturity_rate,
            owner_lock,
        ...
        let siblings_at_anchor = Self::reduce_lock(
            &aggregate_at_anchor,
            individual_at_anchor.locked_mass,
            individual_at_anchor.conviction,
        );

        // Preserve sub-threshold member contributions in the sibling remainder
        // until those members are detached individually. Applying the
        // individual dust threshold to the merged remainder would make a
        // later member reconstruction subtract value that is no longer in the
        // aggregate.
        let rolled_siblings = Self::roll_forward_lock_preserving_dust(
            siblings_at_anchor,
            now,
            unlock_rate,
            maturity_rate,
        ...
        let rolled_individual = Self::roll_forward_lock(
            individual_at_anchor,
            now,
            unlock_rate,
            maturity_rate,
            owner_lock,
    ...
    fn add_to_aggregate(&mut self, added: LockContribution) {
        let (aggregate, aggregate_dirty) = self.aggregate_mut();
    ...
    fn reduce_aggregate(&mut self, removed: LockContribution) {
        let (aggregate, aggregate_dirty) = self.aggregate_mut();
        *aggregate = Self::reduce_lock(aggregate, removed.locked_mass, removed.conviction);
        *aggregate_dirty = true;
    }
    ...
    fn force_reduce_individual(&mut self, amount: AlphaBalance, now: u64) {
        ...
        self.reduce_aggregate(LockContribution {
            locked_mass: locked_mass_diff,
            conviction: conviction_diff,
        });
    }
    ...
    pub fn roll_forward_lock(
        ...
        let mut rolled = Self::roll_forward_lock_preserving_dust(
            lock,
            now,
            unlock_rate,
            maturity_rate,
    ...
    /// Evolves a lock without applying the individual dust threshold.
    ///
    /// Aggregate synchronization uses this to reconstruct and detach the
    /// member's full contribution at the aggregate timestamp. Dust cleanup is
    /// applied after the member and the sibling remainder are separated.
    fn roll_forward_lock_preserving_dust(
        lock: LockState,
        now: u64,
        unlock_rate: u64,
        maturity_rate: u64,
...
impl<T: Config> Pallet<T> {
    ...
    fn add_aggregate_lock(
        ...
        model.add_to_aggregate(LockContribution::from(&added));
        model.roll_forward_aggregate(now, UnlockRate::<T>::get(), MaturityRate::<T>::get());
        Self::save_conviction_model(coldkey, netuid, hotkey, model);
    }
    ...
    fn reduce_aggregate_lock(
        ...
        let mut model = Self::read_conviction_model_for_hotkey(coldkey, netuid, hotkey, now);
        model.roll_forward_aggregate(now, UnlockRate::<T>::get(), MaturityRate::<T>::get());
        model.reduce_aggregate(LockContribution {
            locked_mass: amount,
            conviction,
        });
        Self::save_conviction_model(coldkey, netuid, hotkey, model);
    ...
    /// Rebuild one hotkey's aggregate buckets from its canonical individual rows
    /// using the role it will have after an ownership transition.
    fn rebuild_hotkey_lock_buckets_for_role(
        netuid: NetUid,
        hotkey: &T::AccountId,
        owner_lock: bool,
        now: u64,
        ...
        // Collect before rewriting LockingColdkeys so dust and orphan cleanup cannot
        // disturb the prefix iterator.
        let coldkeys: Vec<T::AccountId> = LockingColdkeys::<T>::iter_prefix((netuid, hotkey))
            .map(|(coldkey, ())| coldkey)
            .collect();
        let mut perpetual_aggregate = Self::empty_lock(now);
        let mut decaying_aggregate = Self::empty_lock(now);

        for coldkey in coldkeys {
            let Some(lock) = Lock::<T>::get((&coldkey, netuid, hotkey)) else {
                Self::maybe_remove_locking_coldkey(hotkey, netuid, &coldkey);
                continue;
            };
            ...
            // Persist the role-adjusted member before deriving its aggregate. In
            // particular, owner promotion must make the full-conviction boost
            // canonical on every individual row rather than only on the aggregate.
            ...
            match LockClass::from_roles(owner_lock, perpetual_lock) {
                LockClass::PerpetualGeneral | LockClass::PerpetualOwner => {
                    perpetual_aggregate = perpetual_aggregate.merge(&rolled);
                }
                LockClass::DecayingGeneral | LockClass::DecayingOwner => {
                    decaying_aggregate = decaying_aggregate.merge(&rolled);
                }
            }
        }

        if owner_lock {
            Self::insert_owner_lock_state(netuid, perpetual_aggregate);
            Self::insert_decaying_owner_lock_state(netuid, decaying_aggregate);
        } else {
            Self::insert_hotkey_lock_state(netuid, hotkey, perpetual_aggregate);
            Self::insert_decaying_hotkey_lock_state(netuid, hotkey, decaying_aggregate);
        }
    }
    ...
    /// outgoing and incoming hotkeys is persisted under its new role, then the
    /// affected aggregates are rebuilt from those rows. Keeping the owner boost
    /// on both representations prevents it becoming orphaned after demotion.
    pub(crate) fn transition_subnet_owner_lock_aggregates(
        ...
        // Discard aggregate-only history for the affected hotkeys. The rows below
        // are the canonical source of truth for the rebuilt buckets.
        OwnerLock::<T>::remove(netuid);
        DecayingOwnerLock::<T>::remove(netuid);
        HotkeyLock::<T>::remove(netuid, old_owner_hotkey);
        DecayingHotkeyLock::<T>::remove(netuid, old_owner_hotkey);
        if old_owner_hotkey != new_owner_hotkey {
            HotkeyLock::<T>::remove(netuid, new_owner_hotkey);
            DecayingHotkeyLock::<T>::remove(netuid, new_owner_hotkey);

            Self::rebuild_hotkey_lock_buckets_for_role(
                netuid,
                old_owner_hotkey,
                false,
                now,
    ...
    pub fn do_move_lock(
        ...
        match Self::read_conviction_model(coldkey, netuid, now) {
            Some((origin_hotkey, mut model)) => {
                ...
                let mut lock = model.detach_individual();
    ...
    pub fn transfer_lock(
        ...
        source_model.detach_individual();
        source_model.attach_to_detached_individual(source_lock);
        ...
        if !locked_transfer.is_zero() || maybe_destination_hotkey.is_some() {
            ...
            Self::save_source_then_update_destination(
                ...
                maturity_rate,
                move |destination_model| {
                    let mut destination_lock = destination_model.detach_individual();
                    ...
                    destination_model.attach_to_detached_individual(destination_lock);
                },
            );
        } else {
