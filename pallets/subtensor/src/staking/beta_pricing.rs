...
impl<T: Config> Pallet<T> {
    ...
    /// One index sweep at the given mark: `(bag_level, stake_level, rows_scanned)`.
    /// NAV-weighted means over every stamped fund with outstanding shares above the
    /// dust floor. Both levels default to 1.0 while no fund qualifies.
    fn beta_index_levels_with(
        ...
        for (hotkey, baseline) in BetaBaseline::<T>::iter() {
            ...
            bag_sum = bag_sum.saturating_add(nav_weight.saturating_mul(marks.display));
            stake_sum = stake_sum.saturating_add(nav_weight.saturating_mul(marks.stake));
        }
        if weight_sum == 0 {
            return (one, one, work);
        }
    ...
    /// its display and stake prices start at the index levels of that block.
    /// Idempotent and cheap once stamped (one `contains_key`). Returns the approximate
    /// scan work performed (rows valued across all funds), for callers that price
    /// deposit weight.
    ///
    ...
    /// holdings, shares, and `BasketRate` are final for the block. Runs at most once
    /// per fund life: the sweep's cost is bounded by total holdings across stamped
    /// funds, and a hotkey only triggers it again by starting a genuinely new fund.
    pub(crate) fn stamp_beta_baseline_if_new(hotkey: &T::AccountId) -> u64 {
        ...
        let (nav, mut work) = Self::basket_realizable_nav_rao(hotkey);
        if nav == 0 {
        ...
        // The sweep only sees stamped funds, so the newborn cannot skew its own splice.
        let mut cache = SpotPriceCache::new();
        let (bag_level, stake_level, sweep_work) =
            Self::beta_index_levels_with(IndexMark::Realizable, &mut cache);
        work = work.saturating_add(sweep_work);
        let mut price_divisor = raw.checked_div(bag_level).unwrap_or(raw);
        if price_divisor == 0 {
