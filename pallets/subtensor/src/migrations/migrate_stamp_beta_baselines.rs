...
/// table's baseline; every baseline that existed before the upgrade is untouched; the
...
pub mod stamp_beta_baselines {
    ...
    /// seed and every baseline that already existed (and must survive verbatim).
    #[cfg(feature = "try-runtime")]
    ...
    impl<T: Config> OnRuntimeUpgrade for Migration<T> {
        ...
        fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
            ...
            Ok((expected, preexisting).encode())
        }

        #[cfg(feature = "try-runtime")]
        fn post_upgrade(state: Vec<u8>) -> Result<(), TryRuntimeError> {
            let (expected, preexisting): PreUpgradeState<T> =
