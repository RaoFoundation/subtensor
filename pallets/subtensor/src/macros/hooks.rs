    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn on_runtime_upgrade() -> frame_support::weights::Weight {
            ...
            weight = weight
                ...
                // Mint the historical root dividend shortfall into the root subnet account and
                // raise SubnetTAO[0] / TotalStake to match root holdings, so every root staker
                // can exit. One-shot, guarded by HasMigrationRun.
                .saturating_add(migrations::migrate_fix_root_pot_shortfall::migrate_fix_root_pot_shortfall::<T>());
