    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn on_runtime_upgrade() -> frame_support::weights::Weight {
            ...
            weight = weight
                ...
                // Drop every stored root basket weight vector and the retired
                // `set_root_weights` gate/cap storage: dividends accumulate in place on
                // their origin subnet and validators rebalance with `swap_basket_alpha`.
