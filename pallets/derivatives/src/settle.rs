//! Adding to and settling positions. Every pool touch goes through `T::Pool`.
//!
//! A position is a sum of tranches, so there are only two primitives here: [`Pallet::lift_tranche`]
//! borrows one more slice of the pool and returns it as a [`Tranche`] to be folded in, and
//! [`Pallet::do_settle`] unwinds a fraction of the position at the current price. `add` is
//! built from those two: it settles when the deposit is on the other side, and grows an
//! existing or empty position otherwise. `close` is `do_settle` with a fraction of one.
//!
//! The chain's own work is [`Pallet::collect_due`]: the positions due this block, each one's
//! interest moved from its cushion into its pool, and any that cannot pay handed back whole
//! by [`Pallet::forfeit`].

use frame_support::storage::with_storage_layer;
use sp_runtime::{Rounding, helpers_128bit::multiply_by_rational_with_rounding};

use crate::{weights::WeightInfo, *};

/// Take up to `want` from `pot`. Returns what was taken; less than `want` means the pot is
/// out.
fn take(pot: &mut TaoBalance, want: TaoBalance) -> TaoBalance {
    let taken = want.min(*pot);
    *pot = pot.saturating_sub(taken);
    taken
}

/// `alpha` in TAO at the price `tao / alpha` given by `price`. Alpha is worth nothing when the
/// swap moved no TAO; it is worth everything when it moved TAO for no alpha.
fn tao_value(
    alpha: AlphaBalance,
    (price_tao, price_alpha): (TaoBalance, AlphaBalance),
    rounding: Rounding,
) -> TaoBalance {
    let value = multiply_by_rational_with_rounding(
        u128::from(alpha.to_u64()),
        u128::from(price_tao.to_u64()),
        u128::from(price_alpha.to_u64()),
        rounding,
    )
    .unwrap_or(if alpha.is_zero() { 0 } else { u128::MAX });
    TaoBalance::from(u64::try_from(value).unwrap_or(u64::MAX))
}

/// The inverse of [`tao_value`]: `tao` in alpha at the same price.
fn alpha_value(
    tao: TaoBalance,
    (price_tao, price_alpha): (TaoBalance, AlphaBalance),
    rounding: Rounding,
) -> AlphaBalance {
    let value = multiply_by_rational_with_rounding(
        u128::from(tao.to_u64()),
        u128::from(price_alpha.to_u64()),
        u128::from(price_tao.to_u64()),
        rounding,
    )
    .unwrap_or(if tao.is_zero() { 0 } else { u128::MAX });
    AlphaBalance::from(u64::try_from(value).unwrap_or(u64::MAX))
}

impl<T: Config> Pallet<T> {
    /// Open, add to, reduce, or flip the caller's position on `netuid`. One storage layer around
    /// the whole call: whichever branch runs, all of it lands or none of it does. The
    /// network-wide switch is checked first: off, nothing is added, not even a reduction, since
    /// a reduction past the flip point would open the other side. The long switch comes next,
    /// on what the call would leave open rather than on the side asked for: a long-side add
    /// that only reduces or closes a short goes through, one that would leave a long does
    /// not. `close` is the exit.
    ///
    /// `min_amount_out` is the caller's floor on the TAO this call pays them, after interest:
    /// the settled share's payout when the add reduces or closes a position, nothing when it
    /// only opens or grows one. Below the floor the whole call fails with
    /// `SettlementBelowMinimum` and nothing moves. Zero is no floor.
    pub(crate) fn do_add(
        owner: T::AccountId,
        netuid: NetUid,
        side: Side,
        deposit: TaoBalance,
        leverage_percent: u16,
        min_amount_out: TaoBalance,
    ) -> DispatchResult {
        with_storage_layer(|| {
            ensure!(
                DerivativesEnabled::<T>::get(),
                Error::<T>::DerivativesDisabled
            );
            let existing = Positions::<T>::get(&owner, netuid);
            let leaves_a_long = side == Side::Long
                && match &existing {
                    Some(short) if short.side() == Side::Short => {
                        Self::flip_surplus(short, deposit, leverage_percent).is_some()
                    }
                    _ => true,
                };
            ensure!(
                !leaves_a_long || LongsEnabled::<T>::get(),
                Error::<T>::LongsDisabled
            );
            ensure!(
                Self::leverage_allowed(side, leverage_percent),
                Error::<T>::LeverageOutOfRange
            );
            let now = frame_system::Pallet::<T>::block_number();
            let (position, deposit) = match existing {
                Some(position) if position.side() != side => {
                    // The floor is checked in `do_settle` against what the reduction or the
                    // close pays.
                    match Self::reduce(
                        &owner,
                        netuid,
                        position,
                        deposit,
                        leverage_percent,
                        min_amount_out,
                    )? {
                        None => return Ok(()),
                        Some(rest) => (Position::empty(side, now), rest),
                    }
                }
                existing => {
                    // An add that only opens or grows pays nothing out, so any floor is unmet.
                    ensure!(min_amount_out.is_zero(), Error::<T>::SettlementBelowMinimum);
                    (
                        existing.unwrap_or_else(|| Position::empty(side, now)),
                        deposit,
                    )
                }
            };
            Self::grow(owner, netuid, position, deposit, leverage_percent)
        })
    }

    /// Exposure an add asks for: `leverage_percent / 100` times `deposit`.
    fn asked_exposure(deposit: TaoBalance, leverage_percent: u16) -> u128 {
        u128::from(deposit.to_u64())
            .saturating_mul(u128::from(leverage_percent))
            .checked_div(100)
            .unwrap_or(0)
    }

    /// What an opposite-side add would open on the other side once `position` is closed: the
    /// part of the deposit past the flip point, or `None` if the add stays within the position
    /// or the part past it is dust below `MinDeposit`. Pure arithmetic on the position as it
    /// stands, so it can be asked before anything is settled.
    fn flip_surplus(
        position: &Position<BlockNumberFor<T>>,
        deposit: TaoBalance,
        leverage_percent: u16,
    ) -> Option<TaoBalance> {
        let asked = Self::asked_exposure(deposit, leverage_percent);
        let held = u128::from(position.exposure_tao.to_u64());
        let rest = asked.checked_sub(held)?;
        let rest_deposit = rest
            .saturating_mul(100)
            .checked_div(u128::from(leverage_percent))
            .unwrap_or(0)
            .min(u128::from(u64::MAX)) as u64;
        let rest_deposit = TaoBalance::from(rest_deposit);
        // Dust past the flip point is not worth a position.
        (rest_deposit >= T::MinDeposit::get()).then_some(rest_deposit)
    }

    /// Lift a tranche for `deposit` and fold it into `position`, which may be empty.
    fn grow(
        owner: T::AccountId,
        netuid: NetUid,
        mut position: Position<BlockNumberFor<T>>,
        deposit: TaoBalance,
        leverage_percent: u16,
    ) -> DispatchResult {
        let side = position.side();
        let tranche = Self::lift_tranche(&owner, netuid, side, deposit, leverage_percent)?;
        let added = Event::PositionAdded {
            owner: owner.clone(),
            netuid,
            side,
            deposit,
            leverage_percent,
            legs: tranche.legs,
            exposure_added: tranche.exposure_tao,
            exposure_tao: position.exposure_tao.saturating_add(tranche.exposure_tao),
        };
        position
            .fold(tranche, frame_system::Pallet::<T>::block_number())
            .ok_or(DispatchError::Corruption)?;
        // A new position joins the interest queue at its first due block; an existing one is
        // already listed and keeps its slot.
        Due::<T>::insert(position.due, (&owner, netuid), ());
        Positions::<T>::insert(&owner, netuid, position);
        OpenByNetuid::<T>::insert(netuid, &owner, ());
        Self::deposit_event(added);
        Ok(())
    }

    /// Take a position out of every index: the book, the subnet index, and the interest queue.
    fn remove(owner: &T::AccountId, netuid: NetUid, position: &Position<BlockNumberFor<T>>) {
        Positions::<T>::remove(owner, netuid);
        OpenByNetuid::<T>::remove(netuid, owner);
        Due::<T>::remove(position.due, (owner, netuid));
        let side = position.side();
        Footprint::<T>::mutate(netuid, side, |f| {
            *f = f.saturating_sub(position.legs.footprint())
        });
    }

    /// Other side: `leverage` times the deposit comes off the position. Less than it holds
    /// settles that share and returns `None`; more closes it and returns the part of the deposit
    /// past the flip point, to open the other side with, or `None` if that part is below
    /// `MinDeposit`. `min_amount_out` is the floor on what the settlement pays.
    fn reduce(
        owner: &T::AccountId,
        netuid: NetUid,
        position: Position<BlockNumberFor<T>>,
        deposit: TaoBalance,
        leverage_percent: u16,
        min_amount_out: TaoBalance,
    ) -> Result<Option<TaoBalance>, DispatchError> {
        let asked = Self::asked_exposure(deposit, leverage_percent);
        ensure!(asked > 0, Error::<T>::ZeroExposure);
        let held = u128::from(position.exposure_tao.to_u64());

        if asked < held {
            // Both fit in u64: `asked < held <= u64::MAX`.
            let fraction = Perquintill::from_rational(asked as u64, held as u64);
            Self::do_settle(owner, netuid, fraction, min_amount_out)?;
            return Ok(None);
        }

        Self::do_settle(owner, netuid, Perquintill::one(), min_amount_out)?;
        Ok(Self::flip_surplus(&position, deposit, leverage_percent))
    }

    /// Take `deposit` from `owner`, lift `phi` of the pool and swap the borrowed half. The
    /// footprint is booked here; the caller folds the rest into a position.
    fn lift_tranche(
        owner: &T::AccountId,
        netuid: NetUid,
        side: Side,
        deposit: TaoBalance,
        leverage_percent: u16,
    ) -> Result<Tranche, DispatchError> {
        ensure!(T::Pool::is_dynamic(netuid), Error::<T>::SubnetNotDynamic);
        ensure!(deposit >= T::MinDeposit::get(), Error::<T>::DepositTooLow);

        let pallet_account = Self::pallet_account();
        let pallet_hotkey = Self::pallet_hotkey()?;
        let params = Params::<T>::get();

        let (tao_reserve, alpha_reserve) = T::Pool::reserves(netuid);
        let (t, a) = (tao_reserve.to_u64(), alpha_reserve.to_u64());
        ensure!(t > 0 && a > 0, Error::<T>::SubnetNotDynamic);
        let (smoothed_tao, smoothed_alpha) = T::Pool::smoothed_reserves(netuid);

        // The slice a spot swap in the same block cannot improve on: no more TAO than the
        // deposit times the leverage, and no more alpha than that TAO is worth at the smoothed
        // price. Whichever bound the live reserves hit first sets `phi`; on an untouched pool
        // both give the same share. The cap is taken at the smaller of the live and smoothed
        // lent reserve.
        let phi = pool_fraction(
            leverage_percent,
            deposit.to_u64(),
            (t, a),
            (smoothed_tao.to_u64(), smoothed_alpha.to_u64()),
        );
        ensure!(!phi.is_zero(), Error::<T>::ZeroExposure);
        ensure!(!phi.is_one(), Error::<T>::PoolCapExceeded);
        let (live_lent, smoothed_lent) = match side {
            Side::Short => (t, smoothed_tao.to_u64()),
            Side::Long => (a, smoothed_alpha.to_u64()),
        };
        let cap = params.pool_share.mul_floor(live_lent.min(smoothed_lent));

        T::Pool::transfer_tao(owner, &pallet_account, deposit)?;

        let (lifted_tao, lifted_alpha) =
            T::Pool::lift_liquidity(netuid, phi, &pallet_account, &pallet_hotkey)?;
        ensure!(!lifted_tao.is_zero(), Error::<T>::ZeroExposure);
        let legs = match side {
            Side::Short => {
                let proceeds = T::Pool::sell_alpha_internal(
                    &pallet_account,
                    &pallet_hotkey,
                    netuid,
                    lifted_alpha,
                )?;
                ensure!(!proceeds.is_zero(), Error::<T>::ZeroExposure);
                Legs::Short {
                    proceeds,
                    debt: lifted_alpha,
                    escrow: lifted_tao,
                }
            }
            Side::Long => {
                let proceeds = T::Pool::buy_alpha_internal(
                    &pallet_account,
                    &pallet_hotkey,
                    netuid,
                    lifted_tao,
                )?;
                ensure!(!proceeds.is_zero(), Error::<T>::ZeroExposure);
                Legs::Long {
                    proceeds,
                    debt: lifted_tao,
                    escrow: lifted_alpha,
                }
            }
        };
        // The cap is checked on what the pool actually lost, `proceeds + escrow` after the
        // opening swap, not on a projection. A constant-product estimate is only right at
        // equal balancer weights; on a TAO-heavy pool a long takes more alpha per TAO than
        // the estimate says, and the cap is what bounds the price move one side can cause.
        let footprint = Footprint::<T>::get(netuid, side).saturating_add(legs.footprint());
        ensure!(footprint <= cap, Error::<T>::PoolCapExceeded);
        Footprint::<T>::insert(netuid, side, footprint);

        // The rate is the side's, read now and fixed for this tranche for as long as it lives.
        Ok(Tranche {
            deposit,
            legs,
            exposure_tao: lifted_tao,
            interest_per_year: params.interest_for(side, lifted_tao),
        })
    }

    /// Unwind `fraction` of the position at the current price: reverse that share of the open
    /// swap, repay the pool plus the whole interest owed so far, pay the owner what is left of
    /// that share of the cushion. A fraction of one closes the position. Atomic. Everything paid
    /// is reported in the event.
    ///
    /// **An underwater share is never traded.** Before the closing swap, the pool's own quote
    /// decides whether the share's pot can repay its debt plus the interest owed. If it
    /// cannot, nothing is swapped: the cushion share, the proceeds, and the escrow go to the
    /// pool in kind, the owner is paid nothing for the share, and the event reports the whole
    /// debt as the shortfall. The owner has already lost everything a swap could cost them, so
    /// a swap at that point would be a market order paid for by the pool, and one that anyone
    /// who saw the close coming could trade against.
    ///
    /// **A partial settlement returns nothing to the pool.** What the pool is owed for the
    /// settled share, the alpha bought back or the TAO repaid, and everything an underwater
    /// share forfeits, is booked in [`Position::held`]; the escrow stays in the legs. All of
    /// it goes back in one return at the full close, after the last buyback. See
    /// [`Position`] for why.
    ///
    /// **`min_amount_out` is the owner's floor on the payout**, checked after the interest
    /// comes off and before anything is paid or returned. A payout below it fails the whole
    /// settlement with `SettlementBelowMinimum`, and the position stays exactly as it was:
    /// an underwater share pays nothing, so with any floor above zero it is not forfeited
    /// either. The settlement trades at the live price, which anyone can move in the same
    /// block ahead of it; the floor is what bounds what that can cost the owner. Zero is no
    /// floor.
    pub(crate) fn do_settle(
        owner: &T::AccountId,
        netuid: NetUid,
        fraction: Perquintill,
        min_amount_out: TaoBalance,
    ) -> DispatchResult {
        with_storage_layer(|| {
            let mut position = Positions::<T>::take(owner, netuid).ok_or(Error::<T>::NoPosition)?;
            let side = position.side();
            let full = fraction.is_one();

            let pallet_account = Self::pallet_account();
            let pallet_hotkey = Self::pallet_hotkey()?;
            let now = frame_system::Pallet::<T>::block_number();
            let interest_due = position.interest_due(now);

            let part = if full {
                position.legs
            } else {
                position.legs.part(fraction)
            };
            // This share's pot: its cushion plus whatever its closing trade leaves. The rest of
            // the cushion stays behind, except to cover interest this share cannot.
            let mut pot = if full {
                position.cushion
            } else {
                TaoBalance::from(fraction.mul_floor(position.cushion.to_u64()))
            };
            let mut rest = position.cushion.saturating_sub(pot);

            // The pool's quote for the closing trade, against what this share and the cushion
            // that stays can pay. A quote that says the debt cannot be covered is the whole
            // decision: the pool's loss is bounded at exactly what the pallet holds either
            // way, and not trading is what keeps it there.
            let covered = match part {
                Legs::Short { proceeds, debt, .. } => {
                    let quote = T::Pool::quote_buy(netuid, debt);
                    quote <= pot.saturating_add(proceeds)
                        && quote.saturating_add(interest_due)
                            <= pot.saturating_add(proceeds).saturating_add(rest)
                }
                Legs::Long { proceeds, debt, .. } => {
                    let quote = T::Pool::quote_sell(netuid, proceeds);
                    pot.saturating_add(quote) >= debt
                        && pot.saturating_add(quote).saturating_add(rest)
                            >= debt.saturating_add(interest_due)
                }
            };

            // What this share owes the pool from its own closing trade: the alpha it bought
            // back or the TAO it repaid, plus whatever it forfeits. The escrow is not in
            // here; it goes back with `held` at the full close only.
            let (mut tao_to_pool, alpha_to_pool, shortfall, interest_paid) = if covered {
                let (mut tao_to_pool, alpha_to_pool, shortfall) = match part {
                    Legs::Short { proceeds, debt, .. } => {
                        pot = pot.saturating_add(proceeds);
                        let (spent, bought) = T::Pool::buy_alpha_for(
                            &pallet_account,
                            &pallet_hotkey,
                            netuid,
                            debt,
                            pot,
                        )?;
                        pot = pot.saturating_sub(spent);
                        // Bought surplus is dust that goes back with the debt.
                        (
                            TaoBalance::ZERO,
                            bought,
                            Lent::Alpha(debt.saturating_sub(bought)),
                        )
                    }
                    Legs::Long { proceeds, debt, .. } => {
                        pot = pot.saturating_add(T::Pool::sell_alpha_internal(
                            &pallet_account,
                            &pallet_hotkey,
                            netuid,
                            proceeds,
                        )?);
                        let repaid = take(&mut pot, debt);
                        (
                            repaid,
                            AlphaBalance::ZERO,
                            Lent::Tao(debt.saturating_sub(repaid)),
                        )
                    }
                };

                // The interest is owed by the whole position. This share pays it first; what it
                // cannot cover comes off the cushion that stays behind.
                let mut interest_paid = take(&mut pot, interest_due);
                interest_paid = interest_paid
                    .saturating_add(take(&mut rest, interest_due.saturating_sub(interest_paid)));

                // The quote said the debt was covered, but the swap is the last word: a share
                // it still left short is underwater, the owner gets nothing for it and
                // everything the pallet still holds for it goes to the pool.
                if !shortfall.is_zero() {
                    tao_to_pool = tao_to_pool.saturating_add(pot);
                    pot = TaoBalance::ZERO;
                }

                // The interest buys alpha that is recycled; what the pool will not swap goes
                // back to it as TAO.
                tao_to_pool =
                    tao_to_pool.saturating_add(Self::burn_interest(netuid, interest_paid));
                (tao_to_pool, alpha_to_pool, shortfall, interest_paid)
            } else {
                // Underwater on the quote: no swap. Everything held for this share goes back
                // in kind, in the token it is held in. The interest the whole position owes is
                // taken from the cushion that stays, as plain TAO; the share's own pot is the
                // pool's already.
                let interest_paid = take(&mut rest, interest_due);
                let (tao_to_pool, alpha_to_pool, shortfall) = match part {
                    Legs::Short { proceeds, debt, .. } => (
                        pot.saturating_add(proceeds).saturating_add(interest_paid),
                        AlphaBalance::ZERO,
                        Lent::Alpha(debt),
                    ),
                    Legs::Long { proceeds, debt, .. } => {
                        (pot.saturating_add(interest_paid), proceeds, Lent::Tao(debt))
                    }
                };
                pot = TaoBalance::ZERO;
                (tao_to_pool, alpha_to_pool, shortfall, interest_paid)
            };

            // The owner's floor, on the payout after interest. Below it nothing has happened:
            // the storage layer drops the trade, and the position is as it was.
            ensure!(pot >= min_amount_out, Error::<T>::SettlementBelowMinimum);

            // Pay the owner before the pool so the last TAO leaving the pallet account is the
            // pool's share; an owner that cannot be paid forfeits to the pool rather than failing
            // the settlement.
            let payout = Self::pay_tao(&pallet_account, owner, pot);
            tao_to_pool = tao_to_pool.saturating_add(pot.saturating_sub(payout));

            if full {
                // One return for everything: this trade's, the escrow, and what the partial
                // settlements before it held back.
                let (escrow_tao, escrow_alpha) = position.escrow_and_held();
                Self::return_to_pool(
                    netuid,
                    tao_to_pool.saturating_add(escrow_tao),
                    alpha_to_pool.saturating_add(escrow_alpha),
                )?;
                Self::remove(owner, netuid, &position);
                Self::deposit_event(Event::PositionClosed {
                    owner: owner.clone(),
                    netuid,
                    side,
                    closed_by: if covered {
                        Closer::Owner
                    } else {
                        Closer::Underwater
                    },
                    payout,
                    interest_paid,
                    shortfall,
                });
                return Ok(());
            }

            // Held back, not returned. The escrow stays in the legs and in the footprint: it
            // is still out of the pool, so it still counts against the cap.
            position.hold(tao_to_pool, alpha_to_pool);
            let settled = part.without_escrow();
            Footprint::<T>::mutate(netuid, side, |f| *f = f.saturating_sub(settled.footprint()));
            position.cushion = rest;
            position.legs = position.legs.minus(&settled);
            position.exposure_tao = position.exposure_tao.saturating_sub(TaoBalance::from(
                fraction.mul_floor(position.exposure_tao.to_u64()),
            ));
            position.interest_per_year = position.interest_per_year.saturating_sub(
                TaoBalance::from(fraction.mul_floor(position.interest_per_year.to_u64())),
            );
            position.interest_owed = interest_due.saturating_sub(interest_paid);
            position.since = now;
            let exposure_tao = position.exposure_tao;
            Positions::<T>::insert(owner, netuid, position);

            Self::deposit_event(Event::PositionReduced {
                owner: owner.clone(),
                netuid,
                side,
                fraction,
                payout,
                interest_paid,
                shortfall,
                exposure_tao,
            });
            Ok(())
        })
    }

    /// Collect the interest of every position due by `now`. Walks the [`Due`] queue slot by
    /// slot from [`NextDue`] within a budget of [`COLLECTIONS_PER_BLOCK`] collections: an empty
    /// slot costs one read and is passed, so the pointer catches up quickly after any stall; a
    /// slot with more positions than the budget allows is finished over the following blocks.
    /// Returns the weight used.
    pub(crate) fn collect_due(now: BlockNumberFor<T>) -> Weight {
        let per_collection = T::WeightInfo::collect_interest();
        let per_slot = T::DbWeight::get().reads(1);
        let mut meter =
            WeightMeter::with_limit(per_collection.saturating_mul(COLLECTIONS_PER_BLOCK.into()));
        let mut slot = NextDue::<T>::get();
        while slot <= now && meter.try_consume(per_slot).is_ok() {
            let mut keys = Due::<T>::iter_key_prefix(slot);
            loop {
                if !meter.can_consume(per_collection) {
                    // Out of room with this slot unfinished: resume here next block.
                    NextDue::<T>::put(slot);
                    return meter
                        .consumed()
                        .saturating_add(T::DbWeight::get().writes(1));
                }
                let Some(key) = keys.next() else {
                    break;
                };
                meter.consume(per_collection);
                Due::<T>::remove(slot, &key);
                let (owner, netuid) = key;
                // A collection that cannot go through is tried again next block; the queue
                // never drops a position.
                if with_storage_layer(|| Self::collect_interest(&owner, netuid, now)).is_err() {
                    Self::defer(&owner, netuid, slot.saturating_add(1u32.into()));
                }
            }
            slot = slot.saturating_add(1u32.into());
        }
        NextDue::<T>::put(slot);
        meter
            .consumed()
            .saturating_add(T::DbWeight::get().writes(1))
    }

    /// Collect one position's interest at `now`: out of its cushion, into its pool, and on to
    /// its next slot in the queue. A cushion that cannot cover it forfeits the position. The
    /// caller has already taken the position out of its current slot.
    ///
    /// A position on a subnet that is no longer tradable, because it is dissolving, is left
    /// alone: it is booked one period ahead with its clock untouched and settles in the
    /// dissolution phase, where its cushion counts in full. A weekly tick that landed in the
    /// dissolution window would otherwise forfeit a position the settlement was about to pay,
    /// while its owner is locked out of closing it.
    pub(crate) fn collect_interest(
        owner: &T::AccountId,
        netuid: NetUid,
        now: BlockNumberFor<T>,
    ) -> DispatchResult {
        let Some(mut position) = Positions::<T>::get(owner, netuid) else {
            // A stale queue entry; nothing to collect.
            return Ok(());
        };
        if !T::Pool::is_dynamic(netuid) {
            position.due = now.saturating_add(INTEREST_PERIOD.into());
            Due::<T>::insert(position.due, (owner, netuid), ());
            Positions::<T>::insert(owner, netuid, position);
            return Ok(());
        }
        let Some(paid) = position.collect(now) else {
            return Self::forfeit(owner, netuid, position);
        };
        Due::<T>::insert(position.due, (owner, netuid), ());
        Positions::<T>::insert(owner, netuid, position);
        let unburned = Self::burn_interest(netuid, paid);
        Self::return_to_pool(netuid, unburned, AlphaBalance::ZERO)
    }

    /// Pay `interest` to the pool as buy pressure: spend it on alpha and recycle the alpha, so
    /// the pool keeps the TAO and the alpha leaves circulation. Returns the TAO the pool would
    /// not swap (dust, or a paused pool), for the caller to hand back as plain TAO instead.
    fn burn_interest(netuid: NetUid, interest: TaoBalance) -> TaoBalance {
        if interest.is_zero() {
            return TaoBalance::ZERO;
        }
        let burned = with_storage_layer(|| -> DispatchResult {
            let pallet_account = Self::pallet_account();
            let pallet_hotkey = Self::pallet_hotkey()?;
            let alpha =
                T::Pool::buy_alpha_internal(&pallet_account, &pallet_hotkey, netuid, interest)?;
            T::Pool::recycle_alpha(&pallet_account, &pallet_hotkey, netuid, alpha)
        });
        match burned {
            Ok(()) => TaoBalance::ZERO,
            Err(_) => interest,
        }
    }

    /// Put a position back in the queue at `slot`, to be collected then instead.
    fn defer(owner: &T::AccountId, netuid: NetUid, slot: BlockNumberFor<T>) {
        Positions::<T>::mutate_exists(owner, netuid, |position| {
            if let Some(position) = position {
                position.due = slot;
            }
        });
        Due::<T>::insert(slot, (owner, netuid), ());
    }

    /// Hand a starved position back to the pool whole: every TAO and every alpha the pallet
    /// holds for it, with no swap. The pool gets its slice back in kind plus the cushion and
    /// whatever earlier partial settlements held for it; the owner gets nothing. Because
    /// nothing is traded, a forfeit moves no price.
    fn forfeit(
        owner: &T::AccountId,
        netuid: NetUid,
        position: Position<BlockNumberFor<T>>,
    ) -> DispatchResult {
        let side = position.side();
        let (escrow_tao, escrow_alpha) = position.escrow_and_held();
        let (tao, alpha) = match position.legs {
            Legs::Short { proceeds, .. } => (
                position
                    .cushion
                    .saturating_add(proceeds)
                    .saturating_add(escrow_tao),
                escrow_alpha,
            ),
            Legs::Long { proceeds, .. } => (
                position.cushion.saturating_add(escrow_tao),
                proceeds.saturating_add(escrow_alpha),
            ),
        };
        Self::return_to_pool(netuid, tao, alpha)?;
        Self::remove(owner, netuid, &position);
        Self::deposit_event(Event::PositionClosed {
            owner: owner.clone(),
            netuid,
            side,
            closed_by: Closer::Starved,
            payout: TaoBalance::ZERO,
            interest_paid: position.cushion,
            shortfall: match side {
                Side::Short => Lent::Alpha(AlphaBalance::ZERO),
                Side::Long => Lent::Tao(TaoBalance::ZERO),
            },
        });
        Ok(())
    }

    /// Hand `tao` and `alpha` the pallet holds back to `netuid`'s pool, unless the pool's spot
    /// price is more than [`PARK_THRESHOLD_PERCENT`] from its moving price. Then the pair is
    /// parked in [`Parked`] instead, to be re-added by [`Pallet::release_parked`] once the spot
    /// is back. A pool that is dissolving takes the pair at once: its reserves are being cashed
    /// out, not traded.
    ///
    /// Re-adding liquidity is price-neutral by construction, which is exactly the problem when
    /// the price was just pushed: the pool would deepen at the pushed price and whoever pushed
    /// it would sell back into the depth. Parking keeps the pool's own slice out of the market
    /// until the market has said where the price is.
    pub(crate) fn return_to_pool(
        netuid: NetUid,
        tao: TaoBalance,
        alpha: AlphaBalance,
    ) -> DispatchResult {
        if tao.is_zero() && alpha.is_zero() {
            return Ok(());
        }
        if T::Pool::is_dynamic(netuid) && Self::spot_is_off(netuid) {
            Parked::<T>::mutate(netuid, |(parked_tao, parked_alpha)| {
                *parked_tao = parked_tao.saturating_add(tao);
                *parked_alpha = parked_alpha.saturating_add(alpha);
            });
            Self::deposit_event(Event::LiquidityParked { netuid, tao, alpha });
            return Ok(());
        }
        T::Pool::return_liquidity(
            netuid,
            tao,
            alpha,
            &Self::pallet_account(),
            &Self::pallet_hotkey()?,
        )
    }

    /// Whether `netuid`'s spot price is outside the band around its moving price within which
    /// returned liquidity may rejoin the pool.
    pub(crate) fn spot_is_off(netuid: NetUid) -> bool {
        spot_is_off_moving(T::Pool::spot_price(netuid), T::Pool::moving_price(netuid))
    }

    /// Re-add `netuid`'s parked pair to its pool, whatever the price. The caller decides when:
    /// `on_idle` once the spot is back within the band, the dissolution hook at once. Nothing
    /// parked, nothing done.
    pub(crate) fn release_parked(netuid: NetUid) -> DispatchResult {
        let (tao, alpha) = Parked::<T>::get(netuid);
        if tao.is_zero() && alpha.is_zero() {
            Parked::<T>::remove(netuid);
            return Ok(());
        }
        with_storage_layer(|| {
            T::Pool::return_liquidity(
                netuid,
                tao,
                alpha,
                &Self::pallet_account(),
                &Self::pallet_hotkey()?,
            )?;
            Parked::<T>::remove(netuid);
            Self::deposit_event(Event::LiquidityReleased { netuid, tao, alpha });
            Ok(())
        })
    }

    /// Try every parked subnet within `limit`, at `WeightInfo::release_parked()` each: a live
    /// pool whose spot is back within the band gets its pair back, one that is dissolving gets
    /// it at once, the rest wait. There is at most one entry per subnet, so the list is short;
    /// the keys are read first so releasing never disturbs the iteration. Returns the weight
    /// used.
    pub(crate) fn release_parked_within(limit: Weight) -> Weight {
        let per_release = T::WeightInfo::release_parked();
        let mut meter = WeightMeter::with_limit(limit);
        if meter.try_consume(T::DbWeight::get().reads(1)).is_err() {
            return meter.consumed();
        }
        let parked: sp_std::vec::Vec<NetUid> = Parked::<T>::iter_keys().collect();
        for netuid in parked {
            if meter.try_consume(per_release).is_err() {
                break;
            }
            if T::Pool::is_dynamic(netuid) && Self::spot_is_off(netuid) {
                continue;
            }
            // A pool that will not take the pair back keeps it parked and is tried again next
            // block; the dissolution hook is the last resort for one that is going away.
            let _ = Self::release_parked(netuid);
        }
        meter.consumed()
    }

    /// Dissolution path: cash settlement at the subnet's [`DissolutionPrices`], with no swap.
    /// A short's alpha debt is charged at `prices.short`; a long's alpha is credited at
    /// `prices.long`. The pool takes back what it lent plus the interest owed; the owner is
    /// paid the rest, or nothing if the position is underwater at that price, as at any other
    /// settlement. Everything returned lands in the reserves the stakers are paid from in the
    /// later phases.
    ///
    /// **A long is paid in alpha first, TAO second.** The pool is owed `debt + interest` in
    /// TAO. The cushion pays what it can; the rest is paid in kind from the long's alpha,
    /// valued at `prices.long` and rounded up. What alpha is left belongs to the owner. The
    /// pool buys it for TAO out of its reserve, at the same price, as far as the reserve
    /// goes; any alpha the reserve cannot buy stays with the owner as stake, at the pallet
    /// hotkey, and is paid out pro rata with every other stake in the later phases. So no long
    /// is ever paid nothing because an earlier one drew the reserve first, and no settlement
    /// can fail for lack of TAO. The order positions are visited in changes only how much of
    /// a long's value arrives as TAO now and how much as alpha later.
    ///
    /// **Rounding favours the pool.** A short's alpha debt is converted to TAO rounding up; a
    /// long's alpha is credited in TAO rounding down and charged in alpha rounding up. The pool
    /// is the party being wound up and cannot come back for a missing rao; the owner's loss is
    /// at most one.
    ///
    /// **Never fails, never blocks.** The position is removed from every index first, so a
    /// failure further down cannot leave it to be visited again and paid twice, and the
    /// caller's loop always terminates. The pool paying out a long's credit and the final
    /// return of liquidity can only fail in a pool that is already broken; a failure there is
    /// logged and the settlement carries on, because one position's mishap must not stop the
    /// subnet from being cleaned up or the stakers from being paid. Whatever cannot reach the
    /// owner stays with the pool.
    pub(crate) fn settle_at_dissolution(
        owner: &T::AccountId,
        netuid: NetUid,
        prices: DissolutionPrices,
    ) {
        // A position can only exist once the hotkey is claimed.
        let Ok(pallet_hotkey) = Self::pallet_hotkey() else {
            OpenByNetuid::<T>::remove(netuid, owner);
            return;
        };
        let Some(position) = Positions::<T>::get(owner, netuid) else {
            OpenByNetuid::<T>::remove(netuid, owner);
            return;
        };
        let side = position.side();
        // Out of the book before any transfer: the caller iterates `OpenByNetuid` until it is
        // empty, so a position must leave it whether or not what follows succeeds.
        Self::remove(owner, netuid, &position);

        let pallet_account = Self::pallet_account();
        let now = frame_system::Pallet::<T>::block_number();
        let interest_due = position.interest_due(now);

        let (payout, interest_paid, shortfall, tao_to_pool, alpha_to_pool) = match position.legs {
            Legs::Short {
                proceeds,
                debt,
                escrow,
            } => {
                // Debt first, then interest, then the underwater rule, as at any settlement.
                // The interest is plain TAO here rather than buy pressure: there is no pool
                // left to buy from, and the reserves it joins are what the stakers are paid.
                let mut pot = position.cushion.saturating_add(proceeds);
                let owed = tao_value(debt, prices.short, Rounding::Up);
                let repaid = take(&mut pot, owed);
                let interest_paid = take(&mut pot, interest_due);
                let shortfall_tao = owed.saturating_sub(repaid);
                if !shortfall_tao.is_zero() {
                    pot = TaoBalance::ZERO;
                }
                let payout = Self::pay_tao(&pallet_account, owner, pot);
                (
                    payout,
                    interest_paid,
                    Lent::Alpha(alpha_value(shortfall_tao, prices.short, Rounding::Up)),
                    position
                        .cushion
                        .saturating_add(proceeds)
                        .saturating_add(escrow)
                        .saturating_sub(payout),
                    AlphaBalance::ZERO,
                )
            }
            Legs::Long {
                proceeds,
                debt,
                escrow,
            } => {
                let price = prices.long;
                let mut pot = position.cushion;
                let owed = debt.saturating_add(interest_due);
                let from_cushion = take(&mut pot, owed);
                // What the cushion could not pay is paid in alpha at the price, rounded up.
                let alpha_owed =
                    alpha_value(owed.saturating_sub(from_cushion), price, Rounding::Up);
                let (mut alpha_to_pool, mut owner_alpha) = if alpha_owed > proceeds {
                    (proceeds, AlphaBalance::ZERO)
                } else {
                    (alpha_owed, proceeds.saturating_sub(alpha_owed))
                };
                let covered = from_cushion
                    .saturating_add(tao_value(alpha_to_pool, price, Rounding::Down))
                    .min(owed);
                let repaid = covered.min(debt);
                let interest_paid = covered.saturating_sub(repaid).min(interest_due);
                let shortfall_tao = debt.saturating_sub(repaid);

                // The pool buys the owner's alpha for TAO as far as its reserve goes.
                if !owner_alpha.is_zero() {
                    let credit = tao_value(owner_alpha, price, Rounding::Down);
                    let drawn = credit.min(T::Pool::reserves(netuid).0);
                    if !drawn.is_zero() {
                        match T::Pool::draw_tao(netuid, &pallet_account, drawn) {
                            Ok(()) => {
                                pot = pot.saturating_add(drawn);
                                let sold = if drawn == credit {
                                    owner_alpha
                                } else {
                                    alpha_value(drawn, price, Rounding::Up).min(owner_alpha)
                                };
                                alpha_to_pool = alpha_to_pool.saturating_add(sold);
                                owner_alpha = owner_alpha.saturating_sub(sold);
                            }
                            Err(error) => log::error!(
                                "derivatives: pool could not pay {drawn:?} for {owner:?} on {netuid:?}: {error:?}"
                            ),
                        }
                    }
                }
                // Alpha the reserve could not buy stays with the owner, as stake at the
                // pallet hotkey, for the stake payout to settle. If even that cannot go
                // through, the pool keeps it.
                if !owner_alpha.is_zero() {
                    match T::Pool::hand_alpha(
                        netuid,
                        &pallet_account,
                        &pallet_hotkey,
                        owner,
                        owner_alpha,
                    ) {
                        Ok(()) => {}
                        Err(error) => {
                            log::error!(
                                "derivatives: could not hand {owner_alpha:?} alpha to {owner:?} on {netuid:?}: {error:?}"
                            );
                            alpha_to_pool = alpha_to_pool.saturating_add(owner_alpha);
                        }
                    }
                }
                let payout = Self::pay_tao(&pallet_account, owner, pot);
                (
                    payout,
                    interest_paid,
                    Lent::Tao(shortfall_tao),
                    // The pot never held more than the cushion plus what was drawn; what did
                    // not reach the owner goes to the pool with the cushion's share.
                    from_cushion.saturating_add(pot.saturating_sub(payout)),
                    alpha_to_pool.saturating_add(escrow),
                )
            }
        };

        // Plus whatever earlier partial settlements held back for the pool.
        let (held_tao, held_alpha) = position.held;
        if let Err(error) = T::Pool::return_liquidity(
            netuid,
            tao_to_pool.saturating_add(held_tao),
            alpha_to_pool.saturating_add(held_alpha),
            &pallet_account,
            &pallet_hotkey,
        ) {
            log::error!(
                "derivatives: could not return liquidity for {owner:?} on {netuid:?}: {error:?}"
            );
        }

        Self::deposit_event(Event::PositionClosed {
            owner: owner.clone(),
            netuid,
            side,
            closed_by: Closer::Dissolution,
            payout,
            interest_paid,
            shortfall,
        });
    }

    /// Returns how much reached `to` (all of it, or nothing).
    fn pay_tao(from: &T::AccountId, to: &T::AccountId, amount: TaoBalance) -> TaoBalance {
        if amount.is_zero() {
            return TaoBalance::ZERO;
        }
        match with_storage_layer(|| T::Pool::transfer_tao(from, to, amount)) {
            Ok(()) => amount,
            Err(_) => TaoBalance::ZERO,
        }
    }
}
