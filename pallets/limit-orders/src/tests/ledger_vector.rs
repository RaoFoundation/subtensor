...
//!
//! ## Why the v1 capture is not an end-to-end order test
//!
...
//! ## v2
//!
//! The second half of this file pins five v2 captures from the same device, taken
//! 2026-08-10. They close the `signer` half of the gap above for real: each message
//! names the capturing device as `signer`, so the recorded signatures go through
//! `verify_readable` itself rather than through a restatement of it. What they still
//! cannot do is clear `validate_order` — their `chain_id`, netuid and expiry belong to
//! the capture environment, not the mock — so the offline-minted vector remains the
//! one that exercises full order acceptance.
//!
//! They also settle the length rule by measurement. Alongside 255/256/257-byte probes
//! captured in the same session, which bracket the switch exactly at
//! `> LEDGER_MAX_SIGN_SIZE`, the 341–524-byte range of real orders shows that no order
//! can reach the byte-exact branch of `verify_readable` — 341 is the floor, and the
//! branch survives only for the shorter `verify_wrapped` payload.
//!
//! The tail of the file pins a *client* defect as a negative test: a provider id
//! rendered from a string with a case-sensitive `0x` strip produces a doubled prefix.
//! The pallet cannot reproduce it — `provider` is an `H256` — so it fails closed, and
...
/// The `signRaw` envelope: `<Bytes>` ++ `message` ++ `</Bytes>`. The device rejects
/// anything without it with `0x6984`.
...
/// Pinned as text so a change to `render_account` fails here and not only inside
/// the message comparison.
...
fn relayers(list: &[&str]) -> frame_support::BoundedVec<AccountId, frame_support::traits::ConstU32<10>>
{
...
fn device_v2_base() -> OrderV2<AccountId> {
    OrderV2 {
        ...
        fee_rate: Perbill::from_parts(8_500_000),
        fee_recipient: account("5GNJqTPyNqANBkUVMN1LPPrxXnFouWXoe2wNSmmEoLctxiZY"),
        relayer: Some(relayers(&["5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty"])),
        max_slippage: Some(Perbill::from_parts(7_500_000)),
        chain_id: 1,
        partial_fills_enabled: true,
...
/// Wrap a captured vector's order and signature into the shape `verify_readable`
/// consumes.
...
/// makes every v2 signature a user has already captured unverifiable, and the
/// failure surfaces as a string diff rather than as an opaque signature rejection.
#[test]
...
/// `verify_readable` is unreachable for orders — 341 bytes is the floor and it is
/// already 85 bytes clear of the threshold. The branch still matters for the
/// shorter `verify_wrapped` payload, which is why this is a test rather than an
/// assertion in the pallet.
#[test]
...
/// The hardware fact the whole branch rests on, now on five v2 payloads spanning
/// 341–524 bytes: each signature verifies over `blake2_256(payload)` and over
/// nothing else. A verifier that checked the raw blob would reject all five.
#[test]
...
/// What the v1 capture could never do: these messages name the signing device as
/// `signer`, so the recorded signatures go through `verify_readable` itself rather
/// than through a hand-rolled restatement of it.
#[test]
...
/// All five were signed by one key, so nothing but the message binds a signature
/// to its order. Transplanting any signature onto any other captured order must
/// fail — this is the injectivity of `render_order` checked against real
/// signatures rather than against digests.
#[test]
...
// `HexDisplay` emits 64 bare lowercase hex characters after it. `0x` + 64 hex,
// always, for every possible input.
//
// So the defect is a client bug that the chain cannot absorb — and it fails
// CLOSED. The two renderings hash to different digests, `verify_readable`
// recomputes from the order rather than trusting the submitted text, and the
// order is rejected as `InvalidSignature`. No substitution, no wrong provider,
// no loss of funds — but a dead order the user cannot diagnose from the error,
// which is why it has to be fixed client-side.
//
// The two vectors below describe the SAME on-chain order — `H256([0xFF; 32])` —
// which is what makes this a clean negative test: the pallet renders both as
// `DEFECT_CORRECTED_MESSAGE`, and the buggy rendering is unreachable.

/// What a client with the case-sensitive strip emits, pinned verbatim from
/// `v2-linked-pct-uppercase-0X-prefix-DEFECT`. **A defect report, not a target.**
...
/// What the pallet renders for the very same order — one prefix. Byte-identical to
/// the `v2-linked-pct-full-provider-max` vector, which feeds the same 32 bytes
/// under a lowercase prefix: both spellings decode to one `H256`, so both must
/// render to one message.
...
/// The order both messages above purport to describe: provider id all `0xFF`,
/// `pct` 100%. `signer` is the vector's account, not the device's.
...
fn defect_rendering_is_unreachable_from_an_h256_provider() {
    new_test_ext().execute_with(|| {
        ...
        // The two pinned strings differ in exactly one thing, so this test cannot
        // silently pass because both drifted the same way.
        assert_eq!(
...
/// The consequence, end to end: a signature the right key produced over the buggy
/// rendering is rejected, while the same key over the pallet's rendering of the
/// same order is accepted. Both are signed here rather than pinned, so the test
/// isolates the doubled prefix — it is the only difference between the two.
#[test]
fn defect_rendering_is_rejected_by_verify_readable() {
    new_test_ext().execute_with(|| {
        // Same order, but signed by a key we hold so the rejection can only come
        // from the message and never from a `signer` mismatch.
        let order = OrderV2 {
            signer: AccountId::new(SOFTWARE_PUBLIC_KEY),
            ..defect_order()
        };
        ...
        // Both payloads are over the limit, so both are signed as the device would.
